...
async fn main() -> anyhow::Result<()> {
    ...
    .await
    {
        Ok((nats_client, js_ctx)) => {
            ...
            tracing::info!(target: "player-service", "outbox relay started (NATS={})", nats_uri);
        }
        Err(e) => {
            tracing::warn!(
    ...
    if let Some(tls_cfg) = tls_config {
        server_builder = server_builder
            .tls_config(tls_cfg)
            .context("tls_config")?;
        tracing::info!(target: "player-service", "mTLS ENABLED — gRPC client cert verification required");
    } else {
        tracing::warn!(target: "player-service", "⚠ mTLS DISABLED — service running insecure gRPC");
    }
