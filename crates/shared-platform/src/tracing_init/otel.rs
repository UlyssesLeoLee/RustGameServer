//! OTel tracing 路径（per P2-9 拆分，2026-09-05）
//!
//! 给生产用：stdout + OTel OTLP exporter + tracing-opentelemetry bridge。
//! 跟 stdout 路径互斥（全局 tracing subscriber 只能 1 个）。
//!
//! **53.12 任务状态** (per 2026-09-04 RGS-OPEN-QA-001 Q-M-03 + WBS WF-1-53.12)：
//! - 当前 OTel SDK bridge 未启用，tracing-opentelemetry link 缺失
//! - `init_otel_exporter_optional` 是占位实现，env 解析 + 校验 + log 告警
//! - 53.12 完成后改回 `opentelemetry_otlp::new_pipeline().install_batch` 即可生效
//!
//! **拆分原因**：原 tracing_init.rs L68-244 (177 行) 混 3 职责（init_tracing 主函数 +
//! init_otel_exporter_optional 条件 + OtelExporterGuard Drop + 公共类型）。拆开后各
//! 块独立演进，53.12 启用 OTel SDK 时只动 otel.rs。
...
use crate::tracing_init::{OtelConfig, TracingError};

/// 初始化 tracing + OTel (per RGS-DTL-100 §7)
pub fn init_tracing(
    service_name: &str,
    service_version: &str,
    deployment_env: &str,
...
pub fn init_tracing_with_otel(
    ...
    tracing::info!(
        target: "tracing_init::otel",
        service = service_name,
        version = service_version,
        env = deployment_env,
        otel_endpoint = %otel_cfg.endpoint,
        "tracing + OTel initialized"
    );
