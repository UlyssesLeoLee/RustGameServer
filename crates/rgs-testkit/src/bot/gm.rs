//! GM 命令注入 (per DDD Review v0.2 §5.1 M4 + DDD Review v0.3.1 §7.3 Phase C)
//!
//! 模拟 erlang C2 (协议 10399 GM 注入), 走 RGS `admin-service::issue_gm_command`
//! mTLS 通道. **wave 3 升级** — 真实 admin mTLS 接入已替换 stub, 走
...
impl GmClient {
    ...
    pub async fn issue(&self, cmd: &str) -> anyhow::Result<GmResponse> {
        ...
        tracing::debug!(
            ...
            "GmClient::issue (wave 3 real mTLS)"
        );
