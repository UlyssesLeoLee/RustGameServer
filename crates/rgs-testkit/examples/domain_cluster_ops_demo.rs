...
async fn main() {
    ...
    println!("    测试代码位置: crates/cluster-ops/src/realm_lifecycle/tests/ut_state_machine.rs\n");
    ...
    nats.publish("cluster.events", br#"{"event":"realm_created","realm_id":"r1","stage":"NewRealm"}"#).await.unwrap();
    nats.publish("cluster.events", br#"{"event":"realm_scaled","realm_id":"r1","stage":"Scale","nodes":5}"#).await.unwrap();
    nats.publish("cluster.events", br#"{"event":"realm_split","realm_id":"r1","stage":"Split","into":["r1","r2"]}"#).await.unwrap();
    nats.publish("cluster.events", br#"{"event":"realm_merged","realm_id":"r1+r2","stage":"Merge"}"#).await.unwrap();
    nats.publish("cluster.events", br#"{"event":"realm_retired","realm_id":"r1","stage":"Retire"}"#).await.unwrap();
    nats.publish("cluster.events", br#"{"event":"realm_archived","realm_id":"r1","stage":"Archive"}"#).await.unwrap();
    ...
    grpc.expect("POST", "/economy.v1.EconomyService/Transfer", 200, br#"{"ok":true,"tx_id":"t1"}"#);
    grpc.expect("POST", "/match.v1.MatchService/CreateRoom", 200, br#"{"ok":true,"room_id":"rm1"}"#);
    grpc.expect("POST", "/social.v1.SocialService/SendMessage", 200, br#"{"ok":true,"msg_id":"m1"}"#);
    grpc.expect("POST", "/admin.v1.AdminService/QueryHealthView", 200, br#"{"ok":true}"#);
