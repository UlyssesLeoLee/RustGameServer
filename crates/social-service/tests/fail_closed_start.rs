...
//!
//! 验证 per RGS-REV-009 V3 L-2: social-service 启动时若 RGS_TLS_DIR 指向不存在路径
...
#[test]
fn social_service_fail_closed_when_tls_dir_invalid() {
    let output = Command::cargo_bin("social-service")
        .expect("locate social-service binary via cargo metadata")
        ...
        .output()
        .expect("spawn social-service binary");
    ...
    assert!(
        ...
        "social-service 应 fail-closed (exit non-zero), 但 exit code = {:?}\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
        output.status.code()
    );
    ...
    assert!(
        combined.contains("fail")
            ...
            || combined.contains("social-service"),
        "output 应包含 fail-closed 标记 (fail/mTLS/TLS/DB/social-service), got:\n{combined}"
    );
}
