...
/// - 默认 0.10（10%）—— 与 Q-M-03 答复对齐
/// - 范围校验：clamp 到 [0.0, 1.0]，非法值回落默认
/// - 容错：env 未设置时也返回默认值
...
mod tests {
    ...
    fn sqlx_tracing_sample_ratio_valid() {
        let _guard = ENV_LOCK.lock().unwrap();
        env::set_var("SQLX_TRACING_SAMPLE_RATIO", "0.15");
        assert!((sqlx_tracing_sample_ratio() - 0.15).abs() < 1e-9);
        env::remove_var("SQLX_TRACING_SAMPLE_RATIO");
    }

    #[test]
    fn sqlx_tracing_sample_ratio_invalid_fallback() {
        let _guard = ENV_LOCK.lock().unwrap();
        env::set_var("SQLX_TRACING_SAMPLE_RATIO", "2.0");
        assert!((sqlx_tracing_sample_ratio() - 0.10).abs() < 1e-9);
        env::remove_var("SQLX_TRACING_SAMPLE_RATIO");
    }
}
