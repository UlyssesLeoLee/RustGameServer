//! iOS 平台 sparse file 预分配（应用沙箱目录）。
//!
//! per **RGS-IMPL-PLAN-CDN-001 v0.1 §2.2**（ios.rs 文件）+ **§6 R3 风险**
//! （iOS / Android 沙箱目录限制）。
//!
//! # 当前状态
//!
//! **M-2063.2 骨架 stub**：直接返回 `Ok(())`；M-2065.6 实测补全：
//! - 路径：`NSTemporaryDirectory()` 或 `Library/Caches/rgs-sdk/downloads/`
//! - 预分配：通过 `NSFileHandle` + `ftruncate` 设置文件大小
//!
//! # 硬约束
//!
...
/// iOS 平台 sparse file 预分配桩（M-2065.6 实测补全）。
#[allow(dead_code)]
