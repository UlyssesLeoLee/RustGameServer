...
//! - W9 L18 10 RPC + 1 内部 + 1 健康检查
//! - 生命周期: 天梯 90d / 休闲 7d / 房间 30d
//!
//! ## 桶 13 实化
//! - entity: ReplayMeta (回放元数据) + Replay (元数据 + 数据) + ReplayInfo (L18 聚合信息)
...
//! - 10 RPC handler 完整业务逻辑 (W9 L18 加 5: GetReplayInfo / DeleteReplay / LikeReplay / UnlikeReplay / CollectReplay)
//! - 1 张表 migration
//!
//! ## 集成
//! - match-service session 结束自动调 SaveReplay (TODO 推 W36+)
//! - 客户端: 回放拉 / 流 / 点赞 / 收集 (per DTL-038 §2 跨域交互)
//!
//! ## DB
...
pub use entity::ReplayInfo;
pub use error::{Error, Result};
pub use repository::{
    InMemoryReplayRepository, Page, PageRequest, PgReplayRepository, ReplayRepository,
};
pub use service::{CollectionsMap, LikesMap, ReplayDomainService, ReplayServiceImpl};
