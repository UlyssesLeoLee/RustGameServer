...
pub trait ShopService: Send + Sync {
    ...
    // 神格许愿 (3)
    async fn wish_list(&self, player_id: String, pool_id: i32) -> Result<WishListOutput>;
...
pub trait SummonService: Send + Sync {
    ...
    async fn summon_info(&self, player_id: String, pool_id: i32) -> Result<SummonInfoOutput>;
    ...
    async fn summon_box_list(&self, player_id: String, box_id: i32) -> Result<SummonBoxListOutput>;
    ...
    async fn summon_banner_list(&self, player_id: String) -> Result<SummonBannerListOutput>;
...
pub trait FundService: Send + Sync {
    ...
    async fn fund_status(&self, player_id: String, fund_id: i32) -> Result<FundStatusOutput>;

    async fn fund_progress(&self, player_id: String, fund_id: i32) -> Result<FundProgressOutput>;
...
pub trait ActivityService: Send + Sync {
    ...
    async fn activity_get_active_holiday_count(&self, player_id: String) -> Result<i32>;
}
...
impl ShopService for ShopServiceImpl {
    ...
    async fn wish_list(&self, _player_id: String, _pool_id: i32) -> Result<WishListOutput> {
    ...
    async fn point_shop_buy(
        ...
        let exchange_id = {
            ...
            repo.exchange_shops
                .values()
                .find(|s| {
                    s.cost_currency == point_type && s.items.iter().any(|i| i.item_id == item_id)
                })
                .map(|s| s.exchange_id)
                .ok_or_else(|| Error::NotFound {
...
impl ActivityService for ShopServiceImpl {
    ...
    async fn activity_list(
        ...
        let mut templates: Vec<ActivityTemplateEntity> = repo
            .activity_templates
            .values()
            .filter(|t| t.enabled && (!active_only || (now >= t.starts_at && now <= t.ends_at)))
            .cloned()
            .collect();
    ...
    async fn activity_progress(
        ...
        let unlocked = tiers
            .iter()
            .find(|t| {
                t.progress_required == state.progress && !state.claimed_tiers.contains(&t.tier)
            })
            .cloned();
    ...
    async fn activity_count_unclaimed_tiers(
        ...
        let unlocked: i32 = tiers
            ...
            .map(|t| {
                if state.claimed_tiers.contains(&t.tier) {
                    0
                } else {
                    1
                }
    ...
    /// 统计玩家可见的活跃 holiday 活动数 (W41 用于 UI 角标)
    async fn activity_get_active_holiday_count(&self, _player_id: String) -> Result<i32> {
        let repo = self.repo.lock().await;
        let now = Utc::now();
        let count = repo
            .activity_templates
