...
use economy_service::service::{EconomyService, EconomyServiceImpl};
...
async fn main() -> anyhow::Result<()> {
    ...
    let service = EconomyServiceImpl::new();

    // 54.1 占位：仅 health check；54.7 业务实施后加实际 gRPC method
    let svc_health = service.health_check().await?;
    tracing::info!(target: "economy-service", "health check: {}", svc_health);

    // 54.1 占位：tonic server 不实际 bind（待 54.2 proto + 54.3 tonic-build）
    tracing::warn!(target: "economy-service", "54.1 骨架：tonic server 占位不 bind；待 54.2-54.3 启用");

    // 阻塞 1 秒后退出（占位行为）
    tokio::time::sleep(std::time::Duration::from_secs(1)).await;
    tracing::info!(target: "economy-service", "exiting (54.1 placeholder)");
