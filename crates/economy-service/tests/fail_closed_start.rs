...
//!
//! 验证 per RGS-REV-009 V3 L-2: economy-service 启动时若 RGS_TLS_DIR 指向不存在路径
...
#[test]
fn economy_service_fail_closed_when_tls_dir_invalid() {
    let output = Command::cargo_bin("economy-service")
        .expect("locate economy-service binary via cargo metadata")
        ...
        .output()
        .expect("spawn economy-service binary");
    ...
    assert!(
        ...
        "economy-service 应 fail-closed (exit non-zero), 但 exit code = {:?}\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
        output.status.code()
    );
    ...
    assert!(
        combined.contains("fail")
            ...
            || combined.contains("economy-service"),
        "output 应包含 fail-closed 标记 (fail/mTLS/TLS/DB/economy-service), got:\n{combined}"
    );
}
