...
async fn main() -> anyhow::Result<()> {
    ...
    .await
    {
        Ok((nats_client, js_ctx)) => {
            ...
            tracing::info!(target: "cluster-ops", "outbox relay started (NATS={})", nats_uri);
        }
        Err(e) => {
            tracing::warn!(
                target: "cluster-ops",
    ...
    if let Some(tls_cfg) = tls_config {
        server_builder = server_builder
            .tls_config(tls_cfg)
            .context("tls_config")?;
        tracing::info!(target: "cluster-ops", "mTLS ENABLED — gRPC client cert verification required");
    } else {
        tracing::warn!(target: "cluster-ops", "⚠ mTLS DISABLED — service running insecure gRPC");
    }
