...
//!
//! 验证 per RGS-REV-009 V3 L-2: cluster-ops 启动时若 RGS_TLS_DIR 指向不存在路径
...
#[test]
fn cluster_ops_fail_closed_when_tls_dir_invalid() {
    let output = Command::cargo_bin("cluster-ops")
        .expect("locate cluster-ops binary via cargo metadata")
        ...
        .output()
        .expect("spawn cluster-ops binary");
    ...
    assert!(
        ...
        "cluster-ops 应 fail-closed (exit non-zero), 但 exit code = {:?}\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
        output.status.code()
    );
    ...
    assert!(
        combined.contains("fail")
            ...
            || combined.contains("cluster-ops"),
        "output 应包含 fail-closed 标记 (fail/mTLS/TLS/DB/cluster-ops), got:\n{combined}"
    );
}
