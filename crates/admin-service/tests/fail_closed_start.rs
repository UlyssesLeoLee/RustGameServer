...
//!
//! 验证 per RGS-REV-009 V3 L-2: admin-service 启动时若 RGS_TLS_DIR 指向不存在路径
...
#[test]
fn admin_service_fail_closed_when_tls_dir_invalid() {
    let output = Command::cargo_bin("admin-service")
        .expect("locate admin-service binary via cargo metadata")
        ...
        .output()
        .expect("spawn admin-service binary");
    ...
    assert!(
        ...
        "admin-service 应 fail-closed (exit non-zero), 但 exit code = {:?}\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
        output.status.code()
    );
    ...
    assert!(
        combined.contains("fail")
            ...
            || combined.contains("admin-service"),
        "output 应包含 fail-closed 标记 (fail/mTLS/TLS/DB/admin-service), got:\n{combined}"
    );
}
