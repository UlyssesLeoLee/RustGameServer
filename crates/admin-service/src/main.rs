...
async fn main() -> anyhow::Result<()> {
    ...
    // ULYS-100 P2-#2: 启动 /metrics HTTP endpoint (per 06_Outbox监控指标草案.md §4 Stage 2)
    // 默认端口 METRICS_PORT (env var), 缺省 50058 (per 6 域 ports.ts 惯例; cluster-ops 50057)
    ...
    // ULYS-100 P2-#2: 启动 OutboxMetricsReporter (15s 周期 SQL 聚合, 更新 5 个 gauge)
    tokio::spawn(
    ...
    .await
    {
        Ok((nats_client, js_ctx)) => {
            ...
            let relay = OutboxRelay::new(outbox_repo, producer, RelayConfig::default(), "admin");
