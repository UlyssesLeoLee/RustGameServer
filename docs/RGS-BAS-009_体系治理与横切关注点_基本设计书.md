...
| 总预算 | 依ASM-001（SRE 1〜2人）与NFR-OP-010（常态运维须在SRE 2人以内成立），取**SRE 2人中用于常态运维的部分**。设常态运维占其工时的50%（其余为改善、自动化、项目工作），则总预算 ＝ 2人 × 160小时/月 × 50% ＝ **160 OLU/月** |

> **总预算取值的理由**：若将SRE工时100%分配给常态运维，则无余量用于自动化改善，运维负荷将只增不减，NFR-OP-010必然在某一时点被突破。保留50%用于改善，是使该上限**长期可持续**的必要条件，而非保守估计。
...
> **这正是RGS-REV-001 F-008所预警的情况被量化确认**：七组文档各自遵守ARC-014，合计已超出NFR-OP-010所允许的运维总量约14%。若不做处置，RSK-002（既有评估为**高概率・大影响**）将在PH-5〜PH-6期间实际发生。
...
**处置后余额**：−22 ＋ 24 ＝ **+2 OLU/月**（勉强为正）

| 结论 | 内容 |
|---|---|
| 判定 | 在**全部执行R-1〜R-6回收措施**的前提下，NFR-OP-010可维持成立，但余量仅+2 OLU，**几乎无冗余** |
| 强制含义 | R-1〜R-4、R-6为**必须执行项**（非改善建议）——它们是新增运维面得以引入的前提条件，应按§3.4纳入对应阶段的完成判定基准 |
| 对今后的约束 | 在完成PH-4实测校准前，**不得**再新增未申领额度的运维面。今后新增中间件时，ARC-014判定通过后仍须在本台账申领，余额不足即否决 |
...
| 检查项 | 实现方式 | 阻断级别 |
|---|---|---|
| **未注册域名段** | 扫描`docs/`下全部文档，提取形如`FR-XXX-nnn`／`NFR-XXX-nnn`的ID，其域名段`XXX`须存在于附件C§7登记表，否则失败 | 阻断（对应GOV-ID-005） |
| **未登记TBD** | 提取全部`TBD-<域>-nnn`，核对**附件D§1.3**存在对应行（域内ID列匹配），否则失败 | 阻断（对应F-002根治） |
| **未登记风险** | 同上，核对**附件D§2.3** | 阻断 |
| **未登记ARC** | 提取全部`ARC-nnn`定义处（`## n.n ARC-nnn：`标题模式），核对附件D§3存在对应ADR行 | 阻断（对应GOV-ADR-002） |
| **未登记领域验收标准** | 提取全部`AC-<域>-nnn`，核对附件C§8 | 阻断（对应GOV-AC-004） |
| **跨文档章节引用失效** | 提取形如`RGS-BAS-00n§x.y`的引用，核对目标文档存在该章节号 | 警告（章节重编号时易误报，人工复核） |
| **OLU台账余额为负** | 解析**附件D§5**台账，合计申领超出总预算时失败 | 阻断（对应ARC-026） |

**归属**：该CI检查纳入RGS-BAS-002§4.2既有CI/CD流水线骨架的lint阶段，与RGS-BAS-004§9埋点规范静态检查并列，**不新建独立流水线**（同GOV-OLU-003的理由）。文档类检查仅在`docs/`路径发生变更时触发，不影响QA-006（CI执行时间15分钟以内）。
