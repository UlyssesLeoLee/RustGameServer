...
## 7. v0.3 部署级更新 (2026-09-01 07:55 JST)

per Ulysses 2026-08-31 22:03 JST "k3s 你可以帮我重启" + 2026-08-31 23:30 JST "A" 决策, Mavis 执行 cluster-reset A 路径但失败。

### 7.1 失败现状 (git 实证)

- k3s server 进程: 启动 (per `ps aux`, 最新 PID 232 或 195/222, 多次自动重启)
- k3s API server: listening (6443+6444)
- etcd-0: **Healthy**
- controller-manager: **Healthy**
- scheduler: Unhealthy (k3s 自启后未完全就绪)
- **节点 `ulyssespc`: "No resources found"** (kubectl get nodes 空) — **SIGTERM 重启无效果**
- 0 个 manifest apply (PLACEHOLDER_NAMESPACE 模板卡住)

### 7.2 关键日志证据 (k3s journalctl)
...
per k3s 8/26 JST HPA 强启动风暴历史 (RGS-OPEN-QA-2026-08-27-k3s-deploy v0.4 §0):
- WSL 单节点 k3s, hostname 变化 / etcd 漂移 / PLEG 死锁
- cluster-reset 后 kubelet 期待手动 register, 单进程 k3s 包含 server+agent 通常自动 join, **这次未自动**
- 8/27 部署 manifest 用了 PLACEHOLDER_NAMESPACE 等模板占位符, Mavis 没有 8/27 部署 SOP 的 sed/kustomize 自动化能力

### 7.4 已完成 + 阻塞项

✅ Mavis 已完成 (不依赖 k3s):
- 5 域 UT+IT+ST+fix 4 阶段全部 commit 落 main (32+ commits, `9a5e5d7` HEAD)
- 5 域 mTLS 证书导出 (`D:/rgs-st-mock/certs/`)
- AGENTS.md + OPEN-QA v0.1/v0.2 + 2 DDD Review + FINAL 汇总 落档
- 4 域 fix (Q1/Q2/Q3/Q4/Q6/Q7) 业务实装到 main
- k3s cluster-reset A 路径脚本落档 (`k3s-cluster-reset-A.sh`, commit `9a5e5d7`)

⏳ SRE 介入 (需 Ulysses 真身):
- k3s 节点 ulyssespc 注册 (建议: 完全卸载 k3s + 重新安装 + 不带 --cluster-reset, 重 apply 8/27 manifest)
- 43 manifest 顺序 apply + PLACEHOLDER_NAMESPACE 替换 (per `docs/deploy/05-deploy-sop.md`)
- 5 域 mTLS 证书重生 (per `phase-0-5-step-4-gen-certs.ps1`)
- 8/29 9:30 + 17:15 两次 secret 命名修订 (per RGS-OPEN-QA-2026-08-27-k3s-deploy v0.4)
- 验证 18 pod 1/1 Running + e2e-smoke baseline ≥10 PASS

⏳ SRE 修好 k3s 后 Mavis 续跑:
- 派 ST-fix worker 续跑 st-11/st-12 mTLS 业务级 ST
- 完成 Q8/Q9/Q10 收尾
- git push origin main (33 commits)
- DDD Review 终审决议 6 项 P1 backlog

### 7.5 新派生约束 (建议 Ulysses 写入 RGS 部署 SOP)

1. **WSL reboot 后 k3s 不会自动恢复** → 必须完全卸载 (`k3s-uninstall.sh`) + 重新安装 + 重 apply manifest
2. **8/27 部署 manifest 用了 PLACEHOLDER_* 模板占位符** → 应改为 kustomize 或 helm template, 不依赖 sed 替换
3. **cluster-reset 不是单节点 k3s 修复方法** → 单节点用完全卸载 + 重装 (per k3s 官方建议)
...
5. **Mavis 处理 k3s 问题的尝试边界** (per 22:03 JST Ulysses "k3s 你可以帮我重启" 授权):
   ...
   - ❌ 不应做: 卸载 k3s, 重 apply 18 manifest (需要完整 SRE 工具链), 修证书, 改 yaml

### 7.6 交接给上游 AI (SRE 角色 / 下一个会话)

本 v0.3 推 k3s 部署恢复给 SRE 团队 / Ulysses 真身。代码层 100% 完成, 32+ commit 在 main HEAD `9a5e5d7`, 等待:
1. SRE 恢复 k3s 节点
2. Mavis 派 ST-fix worker 续跑 st-11/st-12 (Q8/Q9/Q10 业务级 ST)
3. Mavis 跑 `git push origin main` 推 33 commits
4. DDD Review 终审决议 6 项 P1 backlog

---

**v0.3 修订人**: Ulysses(一人公司 12 角色 per DEC-008) — Mavis 接手
**v0.3 审批**: 架构师(Mavis 接手 agent per DEC-008)
