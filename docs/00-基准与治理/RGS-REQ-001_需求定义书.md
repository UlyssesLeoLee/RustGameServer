...
| **1.3** | 2026-08-16 | 架构师 | 李典 | **执行RGS-REQ-013§9提出的正式修订提案CR-001〜008**（终审发现：这批提案自提出后从未真正落地至本文档，导致附件C/D已在引用AC-019／VF-011〜016／FT-011〜014等本文档中并不存在的编号，21份领域文档的地基实际悬空）。具体：①§1.5.3新增域内ID体系说明（CR-001）②§5.1新增第12个子系统符号`CS`客户端SDK（CR-002）③§11.2新增§11.2.1领域文档工作的阶段归属表（CR-003）④§11.3中间件表新增DDoS防护/Secrets管理/SBOM工具链/沙箱脚本引擎4行（CR-004）⑤§12.3新增VF-011〜016（CR-005）⑥§12.4新增FT-011〜014（CR-006）⑦**§15.5新增AC-019领域验收标准聚合项**（CR-007，使附件C§8登记的102项领域验收标准重新获得上线门禁阻断力）⑧§14.1／14.2补充与附件D的权威关系声明，并反映ISS-007（TBD-007目标区域方针）决议完毕（CR-008） | §1.5.3、§5.1、§11.2.1、§11.3、§12.3、§12.4、§14.1、§14.2、§15.5 |
| 1.4 | 2026-08-17 | 架构师 | — | **交叉审核修正**：§14.2"数量提示"的TBD/ISS数字为CR-008执行时（2026-08-16）的静态快照，其后附件D又经多轮域文档登记（GSM/TRD/SUP/INF/IDN/OPT/PLT/VIZ/CAP/PPL/DEP/NEURO追补）持续增长，未回填导致漂移；本次更新为当前实际值（TBD-063、ISS-079含1个保留空号），并同步反映ISS-032/ISS-043均已决议完毕的最新状态 | §14.2 |
| 1.5 | 2026-08-25 | 架构师 | — | 登记RGS-REQ-038（核心传输防丢包强化与周边协议选型，ARC-047）：TBD-006/ISS-006**防丢包子问题部分决议**（Datagram路径新增FEC编解码层，不引入重传），拥塞控制/发送节奏子问题仍未着手，待PH-4 | §14.2 |
...
| `<类>-<域>-nnn` | **领域文档专用的域内ID**（RGS-REQ-013§9 CR-001执行） | FR-GSM-001、NFR-VIZ-003、TBD-CAP-001 |

> **域内ID体系的补充说明**（CR-001执行，处置RGS-REV-001 F-001／ARC-025 GOV-ID-001／002）：领域需求定义书（RGS-REQ-006以降）**可以**使用带域名段的域内ID（如`FR-GSM-001`，域名段`GSM`表示排行榜/任务成就/玩家治理领域），以保持领域内部的可读性。其与本表既有ID体系的关系为：
> - 域名段**必须**先在**附件C§7 ID归属登记表**注册，声明其归属子系统与归属NFR区分后方可使用（GOV-ID-005）
> - 域内的`TBD-<域>-nnn`／`RSK-<域>-nnn`**必须**在附件D分配主编号（`TBD-nnn`／`RSK-nnn`）并登记（GOV-ID-003），使本文档第14章的摘要与附件D保持可聚合
> - 域名段**不得**与既有子系统符号（§5.1）或NFR区分重名（GOV-ID-004）
...
| **CS** | **客户端SDK** | **本系统交付给客户端团队的协议与算法实现**（RGS-REQ-013§9 CR-002执行） | **不适用**（运行于客户端进程内，不持有服务端状态） |

> **重要**：`EC`（玩家经济）将道具与货币**统合于同一限界上下文**。此为架构设计方针ARC-008的决定事项，目的是消除商店购买・交易中本不必要的跨库Saga。

> **`CS`符号的新增说明**（CR-002执行，处置RGS-REV-001 F-001／ARC-025 GOV-ID-006）：X-001既有声明"本系统提供协议规范与参考SDK"，但§5.1原有的11个子系统符号均为**服务端**组件，无法容纳RGS-REQ-012定义的客户端核心SDK这一交付物。故新增第12个符号`CS`。**注意其特殊性**：`CS`运行于客户端进程内，**不**持有服务端权威状态、**不**适用ARC-005〜009等针对服务端的一致性保证；其被纳入子系统体系仅为使`FR-SDK-*`系需求可挂接归属，**不得**据此认为客户端具备任何服务端权威（NFR-SE-001服务器权威原则不因本符号新增而改变）。
...
### 11.2.1 领域文档工作的阶段归属（CR-003执行）

> 本表为RGS-REQ-013§9 CR-003的执行（处置RGS-REV-001 F-007）。§11.2的阶段计划制定于仅有基准文档的时期，此后文档体系扩展至21份领域需求定义书（RGS-REQ-006〜026）。**若不将各领域工作明示挂接至阶段，则"某阶段完成判定基准"实际上不包含该阶段本应交付的领域功能**，PP-004"未达标不得进入下一阶段"将失去约束力。
>
> **本表与§11.2的关系**：§11.2定义各阶段的**基础能力**，本表补充各阶段还须交付的**领域能力**。两者合并构成该阶段的完整完成判定基准。

| 阶段 | 追加的领域工作（出处文档） |
|---|---|
| **PH-1** | 治理闭环落地（RGS-REQ-013／BAS-009，含本文档CR-001〜010的执行）；埋点与日志规范（RGS-REQ-008，ARC-020须自PH-1即落地，否则存量埋点劣化后重构成本剧增）；确定性分级体系（RGS-REQ-014 ARC-030，**独立于智能层是否实施**，对未来任何非确定性组件均适用）；请求处理管道并入挂载脚手架（RGS-REQ-026 ARC-041，须先于新服务大量产生） |
| **PH-2** | 数据库设计标准（RGS-REQ-011／BAS-007）；测试基础设施（RGS-REQ-015，模拟客户端须与PH-2同步可用以支撑其完成判定）；客户端SDK协议层（RGS-REQ-012 PH-1〜PH-2） |
| **PH-3** | 插件热插拔（RGS-REQ-009，ARC-021） |
| **PH-4** | 运维与GM后台管控（RGS-REQ-007／BAS-003）；网络安全（RGS-REQ-010／BAS-006）；网络拓扑容灾（RGS-REQ-020 FR-INF-001〜003）；排行榜/任务成就/邮件/举报/赛季（RGS-REQ-017，全部AC-GSM-*） |
| **PH-5** | 数据分析管线（RGS-REQ-020 FR-INF-010〜014） |
| **PH-6** | 大厅/社交/内购（RGS-REQ-016）；客服工单与支付对账（RGS-REQ-019）；账号身份与第三方登录（RGS-REQ-021）；消息推送与兑换码（RGS-REQ-022）；平台内购与选服（RGS-REQ-023）；GM拓扑可视化画布（RGS-REQ-024） |
| **PH-7** | 玩家间交易（RGS-REQ-018，与§11.2既有"交易"条目对应）；场景分片（既有FR-RT-011） |
| **PH-8** | 全部领域验收标准达成（**AC-019**，见§15.5） |
| **持续/跨阶段** | 功能挂载架构（RGS-REQ-006，每次新增App时适用）；弹性容量规划（RGS-REQ-025，T0〜T3按FR-CAP-003触发条件推进，不绑定单一阶段）；智能决策层（RGS-REQ-014 ARC-027部分，**依附CR-011获批与OLU预算转正，当前不得上线**，见附件D ISS-043） |

> **未纳入本表的既知例外**：RGS-REQ-025的T2/T3容量级别、RGS-REQ-014智能层的实施，均**依附于尚未决议的前置条件**（分别为FR-CAP-003触发条件评审、CR-011与ISS-043），故不预先挂接具体阶段。此为有意的留空，不是遗漏。
...
> 以上4行为RGS-REQ-013§9 CR-004执行（处置RGS-REV-001 F-007，原表制定时尚无RGS-REQ-009/010）。
...
| **VF-011** | **新App挂载的隔离性**（挂载过程中既有功能错误率增量不超阈值） | ARC-018、NFR-MNT-002／003 |
| **VF-012** | **控制平面隔离**（GM控制指令不冲击实时路径） | ARC-019、NFR-OPS-003 |
| **VF-013** | **强制全采集**（关键路径埋点不被采样丢弃） | ARC-020、FR-LOG-040 |
| **VF-014** | **插件故障隔离**（单插件错误不导致宿主进程崩溃） | ARC-021、NFR-PLG-004 |
| **VF-015** | **三引擎一致性重放**（同一输入序列在Bevy/Unity/UE产生一致结果） | ARC-024、NFR-SDK-001 |
| **VF-016** | **插件经济边界**（插件产生永久事实必经FR-EC-003确定请求路径） | FR-GOV-001〜004、ARC-005〜009 |

> VF-011〜016为RGS-REQ-013§9 CR-005执行（处置RGS-REV-001 F-015），使领域文档新增的重点验证项目进入本章统一管理，而非游离于各领域文档内部。
...
| **FT-011** | **GM控制平面过载/误操作** | 爆炸半径限制生效，实时路径不受冲击 | 满足NFR-OPS-003／004（ARC-019） |
| **FT-012** | **跨节点套利尝试**（同一玩家在多节点并发操作经济） | 单一权威仲裁，仅一次生效 | 道具/货币总量差分为0（ARC-005、FR-GOV-030〜033） |
| **FT-013** | **删除编排中断→重新执行** | 幂等清除，重复执行达同一终态 | 无残留个人数据，无需传统补偿（FR-GOV-010〜014） |
| **FT-014** | **密钥轮换过程中的服务连续性** | 轮换不停机，新旧密钥并存窗口内请求均可验证 | 满足NFR-SEC-002（ARC-022） |

> FT-011〜014为RGS-REQ-013§9 CR-006执行（处置RGS-REV-001 F-015），与VF-011〜016同批纳入本章统一管理。
...
> **本节的范围与权威关系**（CR-008执行，处置RGS-REV-001 F-003）：本节仅收录**基准文档范围**的风险（RSK-001〜011）。领域文档范围的风险（RSK-012以降，当前至RSK-052）登记于**附件D§2.3**。**附件D为权威，本节为摘要**——两者不一致时以附件D为准。新增风险**必须**登记于附件D，是否同步至本节摘要依其是否属基准文档范围判断。
>
> **须特别关注的既有高风险**（摘自附件D，供本章读者知悉）：RSK-002（中间件运维负荷超SRE体制）已确认实际发生——附件D§5.2显示OLU预算已超支，处置登记为ISS-032（**待决议**）；RSK-023（智能层上线将使预算进一步转负）对应ISS-043（**待决议**，当前智能层不得上线）。
...
| TBD-006 | QUIC发送节奏・拥塞控制的调整方针 → **防丢包子问题已部分决议**（2026-08-25，RGS-REQ-038 ARC-047：QUIC Datagram路径新增FEC编解码层，不引入重传）；拥塞控制/发送节奏子问题仍**未着手**，待PH-4负载试验 | PH-4 | 实时负责人＋SRE |
| ~~TBD-007~~ | ~~目标区域与数据所在地方针~~ → **已决议**（2026-08-16，ISS-007：**单区域优先**，多区域推迟至RGS-REQ-020 FR-INF-004触发条件满足，见ARC-034） | — | ISS-007决议完毕 |
| TBD-008 | 客户端协议旧版本的接受期限 | PH-4 | 实时负责人 |

> **本节的范围与权威关系**（CR-008执行，处置RGS-REV-001 F-002）：本节仅收录**基准文档范围**的未决事项（TBD-001〜008）。领域文档范围的未决事项（TBD-009以降，当前至TBD-060，以域内ID`TBD-<域>-nnn`形式在各领域文档中表述）登记于**附件D§1.3**。**附件D为权威，本节为摘要**。
>
> **数量提示（2026-08-17交叉审核更新，此前数字为2026-08-16 CR-008执行时的静态快照，未随后续域文档登记增长回填，属已知的同类漂移问题，一并修正）**：截至最新更新，附件D已登记未决事项TBD-001〜063（63项主编号）、问题ISS-001〜079（其中ISS-049为保留空号，实际78项）。ISS-032（OLU预算超支处置）已决议完毕（加预算处置）；ISS-043（智能层开关开启与否）已决议完毕（开发/部署正常推进，开关开启另行决议）；ISS-057（许可结构文档同步）仍**待决议**，须**负责人决议**。
...
| **AC-019** | **附件C§8所登记的全部领域验收标准（`AC-<域>-nnn`）均达成**（RGS-REQ-013§9 CR-007执行，处置RGS-REV-001 F-006／ARC-025 GOV-AC-003。该聚合项使各领域文档自行定义的验收标准重新获得商用上线门禁的阻断力，否则其登记于附件C但不进入本章门禁，形同虚设） |
