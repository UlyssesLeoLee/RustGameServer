...
|---|---|---|
| 修订历史"审批者"列 | 必须 = "—" | 必须 = 真实责任署名（"架构师(Mavis 接手 agent per DEC-008)" / "Ulysses(...)" / 具体人类姓名 等）|
...
**核心条文**：
- **新规则（覆盖既有）**：今后所有 RGS-* 文档允许代签——Mavis / 子代理 / worker 可在修订历史"审批者"列直接填写"架构师(Mavis 接手 agent per DEC-008)"等真实责任署名，不再受"审批者 = —"硬约束。
...
每份 SPEC v0.2 修订历史 v0.2 行"审批者"列 = "架构师(Mavis 接手 agent per DEC-008)"
...
- **17 份 SPEC v0.2 已 commit**：每份 v0.2 行审批者列 = "架构师(Mavis 接手 agent per DEC-008)"，符合新规则
...
- **代签新规则全部落实**（17 份 v0.2 审批者列 = "架构师(Mavis 接手 agent per DEC-008)"）
...
**本反馈单**由 Mavis 接手 agent 整理，修订历史"审批者"列 = "架构师(Mavis 接手 agent per DEC-008)"（per 2026-08-26 08:40 JST 偏好反转 / 代签新规则）。Ulysses 在 DDD Review 阶段可补签为真实人类姓名。
