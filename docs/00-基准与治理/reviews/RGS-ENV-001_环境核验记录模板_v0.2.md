...
|---|---|---|---|---|---|---|
| 1 | **DBA Lead** | **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §2 PostgreSQL 18.4（5 DB 划分 + 双向迁移演练）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 2 | **SRE Lead** | **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §3 K3s / Kubernetes（节点就绪 + CoreDNS + Traefik + Helm + 镜像仓库）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 3 | **Player 域 Lead**（独立）| **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §1 工具链 + §5 跨工具集成（player 域相关）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 4 | **Economy 域 Lead**（独立）| **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §1 + §5（economy 域相关）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 5 | **Match 域 Lead**（独立）| **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §1 + §5（match 域相关）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 6 | **Social 域 Lead**（独立）| **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §1 + §5（social 域相关）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 7 | **Admin 域 Lead**（独立，不兼任 SRE）| **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §1 + §5（admin / COC 域相关）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 8 | **架构师** | **Ulysses（架构师）** | **Ulysses（架构师）** | **2026-08-21** | §5 跨工具集成（sqlx 编译期 + tonic gRPC + OTel + distroless）| ✅ 通过 |
| 9 | **Economy 域 Lead（Q-003 二次确认）** | **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | Q-003 跨 DB Saga 在新环境可跑 | ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 10 | **Platform Engineer** | **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §1 工具链 + §4 锁定依赖 CI（Rust 1.98 + cargo fmt/clippy/deny/audit/llvm-cov）| ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
| 11 | **QA Lead** | **所有者背书 + 待具名责任人：`<签名>`** | `<签名>` | ____-__-__ | §4.3 测试覆盖 + §4 测试可重入 | ☐ 通过 / ☐ 有条件 / ☐ 不通过 |
