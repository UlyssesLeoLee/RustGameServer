...
**审核对象**: `git log 10bd5b1..2fe68b4` (12 commits, 55.15 → 55.21+22)
**审核子代理**: verify-B architecture-consistency-adversarial
**审核时间**: 2026-08-23
**commit 基线**: 2fe68b4
**worktree**: `D:\RustGameServer-worktrees\verify-55-B-arch-consistency`

> 范围勘误：用户原始范围 `ec43377..2fe68b4` 仅 3 commit，与"12 commits"描述不符。
> 实际 12 commit 范围为 `10bd5b1..2fe68b4`（55.15 → 55.21+22），本审核以此为准。
...
|----|-----------------------------|-------------------------------|---------------------|
| player-service | ✓（`Cargo.toml:31`, commit 2fe68b4）| ✗（shared-platform 仅在 `client.rs::ServiceId`、`subject.rs` 中**字符串枚举**提及 player/economy/match/social/admin/cluster-ops，无 crate 依赖） | ✗（`main.rs:20-25` 仅 `use player_service::*`）|
| economy-service | ✓（`Cargo.toml:31`, commit 2fe68b4）| 同上 | ✗（`main.rs:23-33` 仅 `use economy_service::*`）|
| match-service | ✓（`Cargo.toml:31`, commit 2fe68b4）| 同上 | ✗（`main.rs:20-25` 仅 `use match_service::*`）|
| social-service | ✓（`Cargo.toml:31`, commit 2fe68b4）| 同上 | ✗（`main.rs:20-25` 仅 `use social_service::*`）|
| admin-service | ✓（`Cargo.toml:32-35`, commit 2fe68b4）| 同上 | ✗（`main.rs:20-25` 仅 `use admin_service::*`）|
| cluster-ops | ✓（`Cargo.toml:31`, commit 2fe68b4）| 同上 | ✗（`main.rs:20-26` 仅 `use cluster_ops::*`）|

**结论**：6/6 域全部追加 `shared-platform = { path = "../shared-platform" }` 依赖（55.21+22 wire-up commit 2fe68b4）。shared-platform 通过 `ServiceId` 枚举和 `SubjectBuilder::parse` 知道 6 域名字，但仅作为路由元数据，**无 5 域 crate 反向依赖**。5 域之间**不互相 import**。依赖图符合 ARC-008 5 域独立原则。
...
|--------|-------------------|---------------|--------|------|
| player-service `main.rs:65` | n/a（55.17 前是 InMemory）| `let outbox_repo: Arc<PgOutboxRepository>` 显式标注 | ✓ | 2fe68b4 commit |
| economy-service `main.rs:141` | n/a | `let outbox_repo: Arc<PgOutboxRepository>` 显式标注 | ✓ | 2fe68b4 commit |
| match-service `main.rs:64` | n/a | `let outbox_repo: Arc<PgOutboxRepository>` 显式标注 | ✓ | 2fe68b4 commit |
| social-service `main.rs:64` | n/a | `let outbox_repo: Arc<PgOutboxRepository>` 显式标注 | ✓ | 2fe68b4 commit |
| admin-service `main.rs:63` | n/a | `let outbox_repo: Arc<PgOutboxRepository>` 显式标注 | ✓ | 2fe68b4 commit |
| cluster-ops `main.rs:66` | n/a | `let outbox_repo: Arc<PgOutboxRepository>` 显式标注 | ✓ | 2fe68b4 commit |
...
|----|------------------|-----------------|----------|
| player-service | `0002_outbox.sql` (55af339) | n/a | ✓ 唯一 |
| economy-service | `0003_outbox.sql` (55af339) | n/a | ✓ 续号（注释说明：0002 已被 `0002_saga_init.sql` 占用）|
| match-service | `0002_outbox.sql` (55af339) | n/a | ✓ 唯一 |
| social-service | `0002_outbox.sql` (55af339) | n/a | ✓ 唯一 |
| admin-service | `0002_outbox.sql` (55af339) | `0002_audit_prev_hash_unique.sql` (44253af) | **⚠ 冲突：两个 0002_ 文件**（见 CRITICAL-1）|
| cluster-ops | `0002_outbox.sql` (55af339) | n/a | ✓ 唯一 |
...
- **位置**：`crates/admin-service/migrations/`
  - `0001_init.sql`（commit 4ad5f20, 54.4）
  - `0002_audit_prev_hash_unique.sql`（commit 44253af, 55.13）
  - `0002_outbox.sql`（commit 55af339, 55.17）
...
- **位置**：`.env.example`（commit 0c8539d, 55.20）
...
<签名>: <占位 — 待最终 commit>
<worktree>: `D:\RustGameServer-worktrees\verify-55-B-arch-consistency`
<commit 范围>: `10bd5b1..2fe68b4` (12 commits, 55.15 → 55.21+22 P0+收尾)
...
| # | commit | 标题 | 文件数 | 关键变更 |
|---|--------|------|--------|----------|
| 1 | 10bd5b1 | 55.15 5 域+cluster-ops main.rs InMemory→Pg 接线 | 多 | Pg 接入 6 域 main |
| 2 | e1b12c6 | 55.15 merge | merge | 合并 |
| 3 | c14abac | 55.16 client_interceptor trace_id 从 Span 提取 | 1 (grpc_tracing.rs) | OTel trace_id 桥接 |
| 4 | 18ec710 | 55.16 merge | merge | 合并 |
| 5 | 68822d2 | 55.14 RBAC DomainAdmin 缺 scope 显式 deny + scope 边界修复 | 1 (rbac.rs) | RBAC 安全修复 |
| 6 | 0c8539d | 55.20 dev 密码 6 域独立化 | 3 (.env.example + script + doc) | 7 独立密码 |
| 7 | 44253af | 55.13 audit_log FNV-1a → SHA-256 + 事务 | 1+ (admin entity.rs + 0002 migration) | hash 升级 |
| 8 | de282e1 | 55.12 SagaOrchestrator handler 实化 | 1 (saga_orchestrator.rs) | Saga 步进实化 |
| 9 | 55af339 | 55.17 outbox SKIP LOCKED + 事务边界 + 6 域 migration | 8 (outbox.rs + outbox_relay.rs + 6 migration) | outbox 升级 |
| 10 | ec43377 | 55.18 mTLS client_auth_required 实化 | 4 (channel.rs + client.rs + lib.rs + tls.rs) | mTLS 强制校验 |
| 11 | 9556d3a | 55.23 economy main.rs SagaOrchestrator 接线 | 1 (economy main.rs) | Saga 收尾 |
| 12 | ecb4c1d | 55.24 housekeeping pre-existing doctest + clippy | 2 (rgs-testkit + json_logging) | 小修 |
| (额外) | 2fe68b4 | 55.21+22 5 域 main.rs mTLS + outbox 接线 | 6 域 main.rs | 当前 HEAD |

> 注：用户原范围 `ec43377..2fe68b4` 实际只覆盖 #10~12 + HEAD = 3 commit。
> 本审核按 "12 commits" 描述扩展到 `10bd5b1..2fe68b4`（含 55.15 P0 起点）。
...
- 收尾配置：`.env.example`（commit 0c8539d）+ `scripts/generate_dev_passwords.ps1`