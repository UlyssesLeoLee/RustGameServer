...
|---|---|---|---|---|
| 0.1 | 2026-08-19 | 架构师 | ⏳ DDD Review 二审 | 初版制定。基于 RGS-BAS-009 的基本设计，覆盖两级 ID 体系、OLU 运维负荷预算机制、治理闭环 CI 机械校验、4 项横切需求落地等模块间的集成契约 |
| 0.2 (字段级映射) | 2026-08-20 | 架构师 | ⏳ DDD Review 二审 | 对齐正文已说明的字段级映射升级，修正文档元数据版本。 |
| 0.3 (シナリオ/テストデータ) | 2026-09-01 | 架构师（Mavis 接手代签 per DEC-008） | ⏳ DDD Review 二审 | 按 2026-09-01 JST 拍板决策，集成测试 (IT) 与系统测试 (ST) 设计书**必须**在用例表内包含「シナリオ」「テストデータ」2 列。新增 §1.4.5 字段定义、§1.6 命名约定补充 S-NNN / TD-NNN、§3.0 场景集 + 测试数据集；§3.1〜3.8 全部用例表扩展为 12 列 (Pattern A) / 9 列 (Pattern B)，并填入示例场景 S-001〜S-015 + 测试数据 TD-001〜TD-012 |
| **v0.2 升版** | 2026-09-07 12:35 | Ulysses — Mavis 接手 | ⏳ Mavis 自审 (after 8 维度整合, per B3 派生约束 DDD Review 二审流程) | 综合 8 维度升版：1) 8 域扩展 (a5235eb/95e67a6/1134cfd/57edbeb/b6b19b7/1dd9afc/3c79bca/42df673) 2) batch v0.1 (fd122f6/e70ed71/e366ff8/62027c9/eb1e15d) 3) admin-coc Phase B (ae9702d/6c2a786/ab127e4/3695f3b) 4) plugin 集群+app 集群架构 (61cf306/f785f18/5cfd692) 5) flash-mock v0.3 (575f5c9/fdba686/01aee71) 6) 9 域 mTLS 业务级 (d270ab9/d15a0bb) 7) REQ/BDD/DDD v0.2 (39d817b/96e6b3c/554b1ef) 8) cutover 收口 L15-L23 (6c6839e/add4238/PHASE-0-TO-4-FINAL.md)。本主题 IT 设计书覆盖 13 域分层 (5 + 8 域扩展) + batch 域纳入基准。git mv v0.1 → v0.2 保留 rename history |
...
| # | 维度 | v0.1 现状 (9/5 拍板) | v0.2 增量 (9/7 拍板, 主题 00 IT 视角) | 引用 |
|---|---|---|---|---|
| 1 | **8 域扩展** | §2 写 "5 域 + 平台层 + function-plane" | §3 用例集覆盖 13 域 (player / economy / match / social / admin + scene / battle / network / account / sub8 / batch + 平台 + function-plane)；id-registry / OLU / gov-ci 集成契约需对 13 域 ID 段全部生效 | a5235eb / 95e67a6 / 1134cfd / 57edbeb / b6b19b7 / 1dd9afc / 3c79bca / 42df673 |
| 2 | **batch v0.1** | §2 完全没提 batch 域 | §3 增 BATCH-IT-001~005 (cron / task_templates / worker_pool / audit_logger / connector mTLS 业务级) — 治理域与 batch 域集成契约 (凭据 / 审计 / 限流) | fd122f6 / e70ed71 / e366ff8 / 62027c9 / eb1e15d |
| 3 | **admin-coc Phase B** | §3.3 仅提 gm-backend 治理 CI 校验 | §3.3 增 admin-coc §X 集成用例 TST-IT-00-091~095 (coc_policy 决策树 3 场景 + 7 项 admin 域 Lead 真实签字 + gm_handlers.rs L79-129 UT 联动) | ae9702d / 6c2a786 / ab127e4 / 3695f3b |
| 4 | **plugin 集群 + app 集群架构** | §3.4 仅提插件经济边界 FR-GOV-001 | §3.4 增 TST-IT-00-101~103 (阶段 0 mock / 阶段 1 MVP plugin-registry 集成 / 阶段 2 独立更新) + APP-DEPLOY-001 灰度发布 | 61cf306 / f785f18 / 5cfd692 |
| 5 | **flash-mock v0.3** | §5.2 测试环境列 paymock | §5.2 升级 rgs-flash-mock v0.3 60 module + W3 5 报告 + 4 新增回归脚本 (9-domain-mtls / batch-domain / 8-domain-extension / admin-coc) | 575f5c9 / fdba686 / 01aee71 |
| 6 | **9 域 mTLS 业务级** | §2.3 仅提 gRPC API 契约 | §2.3 + §3.8 增 EX-MTLS-9DOMAIN-001 (9 域 mTLS 端到端 11 步客户端模拟器 v3, per W26 Phase 3 收口) + 3 NEW 域 k8s yaml 落档 | d270ab9 / d15a0bb |
| 7 | **REQ/BDD/DDD v0.2 升版** | §1.3 引用 REQ v0.1 | §1.3 增 3 addendum 引用 (业务逻辑逆推 + 协议号映射 + frontend 适配) | 39d817b / 96e6b3c / 554b1ef |
| 8 | **cutover 收口** | §6 仅写静态检查 + 跨语言集成 | §6 + §7 增 cutover L15-L23 (L15 native binary 跨工具链 / L16 主会话统一 commit 拍板 / L17 InMemory 5 域→PgRepository 7 域扩展 / L18 113+43 RPC 补全 / L19 mTLS 业务级=saga 触达 / L20 ca.crt 0 字节 / L21 跨工具链 gRPC Code 解析 / L22 协议码映射表 / L23 4 层自动探针) | 6c6839e / add4238 / PHASE-0-TO-4-FINAL.md |

**主题 00 IT 视角专项增量** (5 + 8 域扩展 + batch 域 + 平台 + function-plane, 共 13 域):
- §1.2 适用范围扩展: 13 域 (5 域 + 8 域扩展 + batch + 平台 + function-plane)
- §2.1 V 模型对应关系: BAS-009 跨 13 域 ID 段全部生效
- §3.0.1 场景集: S-016 增 8 域扩展 ID 段 / S-017 增 batch 域集成场景 / S-018 增 admin-coc 决策树场景 / S-019 增 plugin 集群阶段 0 mock / S-020 增 9 域 mTLS 业务级
- §3.0.2 测试数据集: TD-013~017 增 8 域扩展 ID 段 / TD-018~020 增 batch 域 mTLS 凭据 / TD-021 增 admin-coc coc_policy 配置
- §3.1 增 TST-IT-00-081~090 8 域扩展 ID 段 (sub8 / scene / battle / network / account)
- §3.5 增 TST-IT-00-091~095 admin-coc Phase B 集成
- §3.5 增 TST-IT-00-101~105 plugin 集群 + app 集群
- §3.6 增 BZ-007~010 8 域扩展属性不变条件
- §3.8 增 EX-MTLS-9DOMAIN-001 9 域 mTLS 业务级 E2E
...
- 9/1 batch 域 12 派生约束 (per AGENTS.md §7.2): ✅ §3.0 + §3.5 batch 用例对齐
- 9/1 14:58 JST 拍板选项规则: ✅ 8 维度拍板走 ask_user
- 9/4 17:47 JST 测试脚本+数据归入 mock 项目: ✅ §5.2 rgs-flash-mock 全过
...
1. **8 域扩展详细 WBS**: v0.2 §3 提及, 但 8 域扩展详细 WBS 5-10 task 待 DDD Review 阶段补全 (per 9/5 4 阶段 4-6 周估算)
