...
| 0.2 | 2026-08-25 | 架构师（Ulysses（一人公司 12 角色 per DEC-008）） | — | 同步父 BAS-007 升版至 v0.2 + 补 §10 追溯性表新增的 AC-DBS-001〜004 验收标准映射（在本文档"## 追溯性"后追加"### AC 验收映射（同步 BAS-007 v0.2 §10）"小节，逐条登记 AC 由 DTL-007 哪一节提供核对依据） | 末尾"## 追溯性" |
...
### AC 验收映射（同步 BAS-007 v0.2 §10）

RGS-BAS-007 v0.2 在 §10 追溯性表追加了 AC-DBS-001〜004 共 4 条验收标准与本文档章节的映射。本表登记"AC 由 DTL-007 哪一节提供核对依据 / 排除声明"，不重复 BAS-007 §10 原表：

| AC 编号 | AC 描述（BAS-007 §10 原文摘要） | DTL-007 提供核对依据的章节 | 核对方式 |
|---|---|---|---|
| AC-DBS-001 | 分域RGS-DTL物理DDL章节逐表核对命名/索引/分区标准 | §2（模板DDL逐条注释）、§6（覆盖范围声明：核对主体是各业务域DTL，非本文档） | 评审者按 §2 模板句法对照各业务域DTL实际DDL是否符合 §2/§3/§4 规则；本文档仅提供参照句法与 §6 责任声明 |
| AC-DBS-002 | 迁移脚本幂等性与回滚脚本抽查 | §4（`sqlx-cli` 命令、CI Expand-Contract 静态检查脚本骨架、不可逆迁移声明格式） | 评审者按 §4 给出的 CI 脚本与命令骨架实施"向前迁移+回滚"演练，验证幂等性与 down 脚本可用性 |
| AC-DBS-003 | 备份恢复演练，RTO达标 | §6（明确排除：归 RGS-OPS-001 归档） | DTL-007 不提供核对依据；BAS-007 §6 已声明演练记录归档属 RGS-OPS-001 |
| AC-DBS-004 | 存储过程/触发器审查，确认无业务逻辑入库 | §5（登记表 schema） + §6（"§7 例外机制迄今零命中"声明） | 评审者按 §5 schema 核对各业务域DTL文档中触发表是否为空；本批次全部 DTL（001/025/026/027）均无触发器例外，与 BAS-007 §7 "极窄允许边界" 保持一致 |
