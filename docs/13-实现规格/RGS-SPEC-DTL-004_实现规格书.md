...
| 0.2 | 2026-08-26 | 架构师（Mavis 接手 agent per DEC-008）| — | 对齐源 DTL v0.1→v0.2（追溯性表追加）+ 头表版本号同步为 0.2;**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本(本规格 §2~§7)不重写,新增 §A v0.2 对齐说明。不可代签,审批栏姓名字段由 Ulysses 在字段级 DD Review 后补签 | §A(新增) |
