...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-026 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3)，待 RGS-DTL-026 具名 DD Review |
| 源详细设计 | RGS-DTL-026(本 DTL 今日未升版，SPEC v0.2 为前瞻性草案，见 §A.1) |
| 实现范围 | 社交运营与玩家治理扩展 |
...
本规格把 RGS-DTL-026 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-026 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

实现社交、治理、策略、审计、封禁和运营活动的状态、权限、幂等和回滚。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/social；services/player-governance；crates/policy | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-026 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.1 | 2026-08-26 | 架构师 | — | 初版(沿用 v0.1 草稿) | 全部 |
| 0.2 | 2026-08-26 | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 对齐源 DTL-026 当前版本(`0.4`) + 头表 0.2 + 新增 §A v0.2 对齐说明;**不引入新设计**;**代签已允许**(per 2026-08-26 08:40 JST 偏好反转);本 SPEC 为 17 份未升版 DTL 前瞻性 v0.2 草案之一(per RGS-OPEN-QA-2026-08-26-SPEC-v0.2 Q2) | §A(新增) |
...
- **源 DTL**:RGS-DTL-026
- **源 DTL 今日状态**:`0.4`(`2026-08-25`)
- **源 DTL 升版路径**:**今日未升版**(`git log --since="2026-08-26 00:00" --until="2026-08-26 23:59" -- docs/**/RGS-DTL-026_*.md` 无 commit)
- **源 DTL 升版类型**:**前瞻性草案**(非"今日升版沉淀")
- **核心要点**:源 DTL 末次升版 v0.4(2026-08-25,per RGS-OPEN-QA-001 Q-D-10 + ACTIONS-v0.3 A-10)落实扩圈算法的性能边界与降级路径:§4.1 补三段(§4.1.1 n≤500 临时占位 / §4.1.2 降级策略——拆分撮合轮 + 熔断 / §4.1.3 benchmark 子任务契约);`RatingSettlement.calculate()` 公式与 `match_ratings` schema 未变(v0.3 已稳定)。v0.3(2026-08-20)修正 Glicko-2 结算只持久化 rating/RD 而遗漏 volatility 的不一致:`RatingUpdate` 返回三项状态;新增结算幂等回执表(`rating_settlement_receipts`)。v0.2(2026-08-17)新增§7 解决评分算法最终选型(Glicko-2),`match_ratings` 新增 `volatility` 列。本 SPEC 今日(2026-08-26)无新升版对齐,仅做元数据对齐 + §A 缺口列项。

### A.2 对本 SPEC 的影响(实现侧)

| 维度 | v0.1 | v0.2 调整 |
|---|---|---|
| 实现范围 | 与源 DTL v0.1 同步 | 与源 DTL `0.4` 同步(范围不变，仅元数据对齐) |
| 源 DTL 真源 | RGS-DTL-026 v0.1 | RGS-DTL-026 `0.4`(具体修订见 §A.1) |
...
- 源 DTL 修订历史条目:见 RGS-DTL-026 §修订历史表(本 DTL 今日未升版,引用最新一次历史升版 v0.4 / 2026-08-25)
- 父 BAS 升版条目:见对应父 RGS-BAS-026 §修订历史表(本 DTL 对应父 BAS,本日是否升版需自审)
