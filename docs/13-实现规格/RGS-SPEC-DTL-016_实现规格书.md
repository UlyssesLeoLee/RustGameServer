...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-016 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3),待 RGS-DTL-016 具名 DD Review |
| 源详细设计 | RGS-DTL-016(2026-08-26 当日升版对齐,见 §A.1) |
| 实现范围 | 数据经济与交易扩展 |
...
本规格把 RGS-DTL-016 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-016 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

按 DTL-016 的字段、API、异步传播和失败补偿实现；跨域事实只通过既定 contract/event/workflow 传播。
...
- RGS-DTL-016 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.1 | 2026-08-21 | 架构师 | — | 首版:与源 RGS-DTL-016 v0.1~v0.3 一对一映射的骨架规格 | 全部 |
| 0.2 | 2026-08-26 | 架构师（Ulysses（一人公司 12 角色 per DEC-008））| Ulysses（2026-08-26，per RGS-REV-004 字段级 DD Review） | 对齐源 DTL v0.2→v0.3（实质重写+装饰性）+ 头表版本号同步为 0.3;**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本(本规格 §2~§7)不重写,新增 §A v0.2 对齐说明。不可代签,审批栏姓名字段由 Ulysses 在字段级 DD Review 后补签 | §A(新增) |

---

## A. v0.2 对齐说明（2026-08-26,基于源 DTL 今日升版沉淀）

> **本节定位**:把源 RGS-DTL-016 v0.2→v0.3 的"今天增量"沉淀为本 SPEC 的实现侧要求。**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本 §1~§8 不重写,新增内容仅本节。
...
- **源 DTL 升版类型**:实质重写+装饰性
- **核心要点**:v0.2 新增 §3.4 Saga 步骤编号映射 + v0.1 §3.4 异常分支改名为 §3.5(腾位置);v0.3 同步父 BAS-016 v0.3 元数据
...
| 源 DTL 真源 | RGS-DTL-016 v0.1 | RGS-DTL-016 0.3(具体修订见 §A.1) |
...
- 源 DTL 升版内容仅为 实质重写+装饰性(元数据 / 装饰性 / 追溯性追加)时,本节无新缺口继承。
...
- 源 DTL 修订历史条目:见 RGS-DTL-016 §修订历史表
