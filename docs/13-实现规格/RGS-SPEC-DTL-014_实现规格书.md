...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-014 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3),待 RGS-DTL-014 具名 DD Review |
| 源详细设计 | RGS-DTL-014(2026-08-26 当日升版对齐,见 §A.1) |
| 实现范围 | 社交运营、活动与玩家治理 |
...
本规格把 RGS-DTL-014 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-014 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

活动、运营操作、玩家状态和审计按 DTL-014 的状态机、幂等、限流和回滚实现。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/social；services/operations；crates/policy | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-014 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.1 | 2026-08-21 | 架构师 | — | 首版:与源 RGS-DTL-014 v0.1~v0.2 一对一映射的骨架规格 | 全部 |
...
> **本节定位**:把源 RGS-DTL-014 v0.1→v0.2 的"今天增量"沉淀为本 SPEC 的实现侧要求。**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本 §1~§8 不重写,新增内容仅本节。
...
- **源 DTL 升版类型**:轻量合并
- **核心要点**:同步父 BAS-014 升版至 v0.2(装饰性升版,正文本不重写)
...
| 源 DTL 真源 | RGS-DTL-014 v0.1 | RGS-DTL-014 0.2(具体修订见 §A.1) |
...
- 源 DTL 修订历史条目:见 RGS-DTL-014 §修订历史表
