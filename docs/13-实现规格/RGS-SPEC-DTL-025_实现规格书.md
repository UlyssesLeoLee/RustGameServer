...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-025 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3)，待 RGS-DTL-025 具名 DD Review |
| 源详细设计 | RGS-DTL-025(本 DTL 今日未升版,SPEC v0.2 为前瞻性草案,见 §A.1) |
| 实现范围 | 大厅社交通信与运营活动 |
...
本规格把 RGS-DTL-025 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-025 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

实现通信、活动、玩家状态、消息可靠性和运营边界；高频消息不逐条创建 trace，按连接/批次聚合。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/social；crates/chat；crates/activity | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-025 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.1 | 2026-08-21 | 架构师 | — | 首版：与源 RGS-DTL-025 v0.1~v0.3 一对一映射的骨架规格 | 全部 |
| 0.2 | 2026-08-26 | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 对齐源 DTL-025 当前版本(0.3) + 头表 0.2 + 新增 §A v0.2 对齐说明;**不引入新设计**;**代签已允许**(per 2026-08-26 08:40 JST 偏好反转);本 SPEC 为 17 份未升版 DTL 前瞻性 v0.2 草案之一(per RGS-OPEN-QA-2026-08-26-SPEC-v0.2 Q2) | §A(新增) |
...
- **源 DTL**:RGS-DTL-025
- **源 DTL 今日状态**:`0.3`(`2026-08-20`)
- **源 DTL 升版路径**:**今日未升版**(`git log --since="2026-08-26 00:00" --until="2026-08-26 23:59" -- docs/**/RGS-DTL-025_*.md` 无 commit)
- **源 DTL 升版类型**:**前瞻性草案**(非"今日升版沉淀")
- **核心要点**:源 DTL 末次升版为 2026-08-20 v0.3(Rhai 候选 DSL 增补 + ADR-0020 状态明确 + 签名制品/沙箱/版本 fencing/分区 fail-closed/回滚审计/测试追溯补齐),引用该 v0.3 作为本 SPEC v0.2 的对齐基线
...
| 源 DTL 真源 | RGS-DTL-025 v0.1 | RGS-DTL-025 `0.3`(具体修订见 §A.1) |
...
- 源 DTL 修订历史条目:见 RGS-DTL-025 §修订历史表(本 DTL 今日未升版,引用最新一次历史升版 v0.3 @ 2026-08-20)
