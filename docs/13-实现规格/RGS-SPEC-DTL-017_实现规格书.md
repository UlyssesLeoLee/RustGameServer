...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-017 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3),待 RGS-DTL-017 具名 DD Review |
| 源详细设计 | RGS-DTL-017(2026-08-26 当日升版对齐,见 §A.1) |
| 实现范围 | 数据分析管线与读取模型 |
...
本规格把 RGS-DTL-017 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-017 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

分析读取与故障诊断可观测性分离；实现数据分类、延迟、重试、保留、权限和数据质量指标。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/analytics；crates/analytics；deploy/analytics | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-017 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.1 | 2026-08-21 | 架构师 | — | 首版:与源 RGS-DTL-017 v0.1~v0.2 一对一映射的骨架规格 | 全部 |
...
> **本节定位**:把源 RGS-DTL-017 v0.1→v0.2 的"今天增量"沉淀为本 SPEC 的实现侧要求。**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本 §1~§8 不重写,新增内容仅本节。
...
- **源 DTL 升版类型**:轻量合并
- **核心要点**:同步父 BAS-017 升版至 v0.2(装饰性升版,正文本不重写)
...
| 源 DTL 真源 | RGS-DTL-017 v0.1 | RGS-DTL-017 0.2(具体修订见 §A.1) |
...
- 源 DTL 修订历史条目:见 RGS-DTL-017 §修订历史表
