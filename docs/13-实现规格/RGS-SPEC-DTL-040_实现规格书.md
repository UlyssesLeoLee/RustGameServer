...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-040 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3),待 RGS-DTL-040 具名 DD Review |
| 源详细设计 | RGS-DTL-040(本 DTL 今日未升版,SPEC v0.2 为前瞻性草案,见 §A.1) |
| 实现范围 | Admin 域 App、admin_db 与 ClusterOps |
| 目标基线 | Rust 1.98 stable（当前基线；环境/CI Gate）、Actix Web 4.14.1、PostgreSQL 18.6；环境需先核验 |
| 规格真源 | 源 DTL 的接口、字段、状态机、错误码、SQL/proto 和非目标 |

## 1. 使用规则

本规格把 RGS-DTL-040 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-040 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

实现 Admin API、admin_db、RBAC、审批、审计、ClusterOps 控制面和回滚；高风险操作强制捕获并独立审计。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/admin；crates/admin；migrations/admin；deploy/admin | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-040 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.2 | 2026-08-26 | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 对齐源 DTL-040 当前版本(`0.1`) + 头表 0.2 + 新增 §A v0.2 对齐说明;**不引入新设计**;**代签已允许**(per 2026-08-26 08:40 JST 偏好反转);本 SPEC 为 17 份未升版 DTL 前瞻性 v0.2 草案之一(per RGS-OPEN-QA-2026-08-26-SPEC-v0.2 Q2) | §A(新增) |
...
- **源 DTL 升版路径**:**今日未升版**(`git log --since="2026-08-26 00:00" --until="2026-08-26 23:59" -- docs/**/RGS-DTL-040_*.md` 无 commit)
- **源 DTL 升版类型**:**前瞻性草案**(非"今日升版沉淀")
- **核心要点**:源 DTL 仍为 v0.1 契约骨架,§6 列出 4 项待补齐(AdminService→ClusterOpsService 字段级 proto + 错误映射、`admin_db` 迁移脚本/唯一约束/索引评审、`cluster_operator/cluster_admin` 与既有 RBAC 最终矩阵、COC UI 写操作/DLQ replay/PFAU 演练验收证据);本 SPEC v0.2 仅做元数据对齐,§6 缺口继承待 DDD Review 阶段处置
...
| 源 DTL 真源 | RGS-DTL-040 v0.1 | RGS-DTL-040 `0.1`(具体修订见 §A.1) |
...
- 源 DTL v0.1 §6 列出的 4 项待补齐(AdminService→ClusterOpsService 字段级 proto + 错误映射 / `admin_db` 迁移脚本/唯一约束/索引评审 / `cluster_operator/cluster_admin` 与既有 RBAC 最终矩阵 / COC UI 写操作/DLQ replay/PFAU 演练验收证据)仍待 DDD Review 阶段与父 BAS (RGS-BAS-003 / RGS-BAS-031) 逐条对账,本 SPEC §3 / §6 / §7 不得在该 4 项处置前标 Done。
- 本 SPEC v0.2 自审 0 项发现,**等 DDD Review 阶段再行检查**。

### A.4 引用链与证据

- 源 DTL 修订历史条目:见 RGS-DTL-040 §修订历史表(本 DTL 今日未升版,引用最新一次历史升版)
- 父 BAS 升版条目:见对应父 RGS-BAS-003 / RGS-BAS-031 §修订历史表(本 DTL 对应父 BAS,本日是否升版需自审)
