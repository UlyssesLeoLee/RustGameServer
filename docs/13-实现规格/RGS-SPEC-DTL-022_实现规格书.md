...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-022 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3),待 RGS-DTL-022 具名 DD Review |
| 源详细设计 | RGS-DTL-022(2026-08-26 当日升版对齐,见 §A.1) |
| 实现范围 | 核心算法、容量与实时 Runtime |
| 目标基线 | Rust 1.98 stable（当前基线；环境/CI Gate）、Actix Web 4.14.1、PostgreSQL 18.6；环境需先核验 |
| 规格真源 | 源 DTL 的接口、字段、状态机、错误码、SQL/proto 和非目标 |

## 1. 使用规则

本规格把 RGS-DTL-022 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-022 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

实现算法、tick、容量分桶、过载和降级；不得在热路径同步访问 DB、message、workflow 或 telemetry exporter。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | crates/core；services/runtime；crates/capacity | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-022 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.1 | 2026-08-21 | 架构师 | — | 首版:与源 RGS-DTL-022 v0.1~v0.3 一对一映射的骨架规格 | 全部 |
| 0.2 | 2026-08-26 | 架构师（Ulysses（一人公司 12 角色 per DEC-008））| Ulysses（2026-08-26，per RGS-REV-004 字段级 DD Review） | 对齐源 DTL v0.2→v0.3（实质重写）+ 头表版本号同步为 0.3;**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本(本规格 §2~§7)不重写,新增 §A v0.2 对齐说明。不可代签,审批栏姓名字段由 Ulysses 在字段级 DD Review 后补签 | §A(新增) |

---

## A. v0.2 对齐说明（2026-08-26,基于源 DTL 今日升版沉淀）

> **本节定位**:把源 RGS-DTL-022 v0.2→v0.3 的"今天增量"沉淀为本 SPEC 的实现侧要求。**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本 §1~§8 不重写,新增内容仅本节。
...
- **源 DTL 升版类型**:实质重写
- **核心要点**:同步父 BAS-022 升版至 v0.2(§2.3 三类组件扩容手段/§3.3 分片新增下线/§5 容量级别可用性);追溯性新增 §3.3 引用行,正文本对既有 §2/§5 落实内容不重写
...
| 源 DTL 真源 | RGS-DTL-022 v0.1 | RGS-DTL-022 0.3(具体修订见 §A.1) |
...
- 源 DTL 升版内容仅为 实质重写(元数据 / 装饰性 / 追溯性追加)时,本节无新缺口继承。
...
- 源 DTL 修订历史条目:见 RGS-DTL-022 §修订历史表
