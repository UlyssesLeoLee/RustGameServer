...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-039 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口（见 §A.3），待 RGS-DTL-039 具名 DD Review |
| 源详细设计 | RGS-DTL-039（本 DTL 今日未升版，SPEC v0.2 为前瞻性草案，见 §A.1） |
| 实现范围 | Social 域 App 与 social_db |
| 目标基线 | Rust 1.98 stable（当前基线；环境/CI Gate）、Actix Web 4.14.1、PostgreSQL 18.6；环境需先核验 |
| 规格真源 | 源 DTL 的接口、字段、状态机、错误码、SQL/proto 和非目标 |

## 1. 使用规则

本规格把 RGS-DTL-039 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-039 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

实现社交关系、消息/活动、权限、事件、存储和审计；社交状态的读取模型与写入权威边界清晰。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/social；crates/social；migrations/social | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-039 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
| 版本 | 日期 | 制定者 | 审批者 | 变更摘要 | 影响范围 |
|---|---|---|---|---|---|
| 0.1 | 2026-08-21 | 架构师 | — | 建立 SPEC 初稿：实现规则、实现单元、实现契约、可观测性、安全/容错/发布、测试、DoD、Gate 证据 | §1~§8 全章 |
| 0.2 | 2026-08-26 | 架构师（Ulysses（一人公司 12 角色 per DEC-008）） | 架构师（Ulysses（一人公司 12 角色 per DEC-008）） | 对齐源 DTL-039 当前版本（0.1）+ 头表 0.2 + 新增 §A v0.2 对齐说明；**不引入新设计**；**代签已允许**（per 2026-08-26 08:40 JST 偏好反转）；本 SPEC 为 17 份未升版 DTL 前瞻性 v0.2 草案之一（per RGS-OPEN-QA-2026-08-26-SPEC-v0.2 Q2） | §A（新增） |
...
- **源 DTL**：RGS-DTL-039
- **源 DTL 今日状态**：`0.1`（`2026-08-21`，制定日）
- **源 DTL 升版路径**：**今日未升版**（`git log --since="2026-08-26 00:00" --until="2026-08-26 23:59" -- docs/07-社交运营与玩家治理/RGS-DTL-039_*.md` 无 commit）
- **源 DTL 升版类型**：**前瞻性草案**（非"今日升版沉淀"）
- **核心要点**：源 DTL v0.1 起草日（2026-08-21）即声明 §6「待补齐项」含 4 项（关系/队伍/举报/消息物理 DDL、跨域事件 schema 与数据最小化规则、沙箱白名单 API 与规则版本 fencing、Social 集成测试与故障注入矩阵）。本 SPEC v0.2 不变更 §6 引用，仍以源 DTL v0.1 §6 作为下游实现侧待补齐基线。
...
| 源 DTL 真源 | RGS-DTL-039 v0.1 | RGS-DTL-039 `0.1`（具体修订见 §A.1） |
...
- 源 DTL v0.1 §6 已自声明 4 项「待补齐项」（关系/队伍/举报/消息物理 DDL；跨域事件 schema 与数据最小化规则；沙箱白名单 API 与规则版本 fencing；Social 集成测试与故障注入矩阵）。该 4 项由源 DTL v0.1 起草时即声明，非今日升版新增，**本 SPEC 不在 v0.2 处置**，DDD Review 阶段与父 BAS-013 / 上位 REQ-017 逐条对账后处置。
- 本 SPEC v0.2 自审 0 项发现，**等 DDD Review 阶段再行检查**。

### A.4 引用链与证据

- 源 DTL 修订历史条目：见 RGS-DTL-039 §修订历史表（本 DTL 今日未升版，引用最新一次历史升版 v0.1 2026-08-21）
- 父 BAS 升版条目：见对应父 RGS-BAS-013 §修订历史表（父 BAS-013 今日未升版，最近一次升版为历史版本，引用以源 DTL §父文档 字段为准）
