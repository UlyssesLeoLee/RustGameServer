...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-013 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3),待 RGS-DTL-013 具名 DD Review |
| 源详细设计 | RGS-DTL-013(2026-08-26 当日升版对齐,见 §A.1) |
| 实现范围 | 社交与运营服务 |
...
本规格把 RGS-DTL-013 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-013 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

按 DTL-013 的社交域 API、状态、持久化、权限和事件契约实现；玩家标识只进日志/trace/event，不进指标标签。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/social；crates/social；proto/social | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-013 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
|---|---|---|---|---|---|
| 0.1 | 2026-08-21 | 架构师 | — | 首版:与源 RGS-DTL-013 v0.1~v0.3 一对一映射的骨架规格 | 全部 |
| 0.2 | 2026-08-26 | 架构师（Ulysses（一人公司 12 角色 per DEC-008））| Ulysses（2026-08-26，per RGS-REV-004 字段级 DD Review） | 对齐源 DTL v0.1→v0.3（元数据复核）+ 头表版本号同步为 0.3;**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本(本规格 §2~§7)不重写,新增 §A v0.2 对齐说明。不可代签,审批栏姓名字段由 Ulysses 在字段级 DD Review 后补签 | §A(新增) |

---

## A. v0.2 对齐说明（2026-08-26,基于源 DTL 今日升版沉淀）

> **本节定位**:把源 RGS-DTL-013 v0.1→v0.3 的"今天增量"沉淀为本 SPEC 的实现侧要求。**不引入新设计**——仅落实/复核源 DTL 与父 BAS 既有内容,正文本 §1~§8 不重写,新增内容仅本节。

### A.1 源 DTL 今日升版增量

- **源 DTL**:RGS-DTL-013
- **源 DTL 今日状态**:0.3(2026-08-25)
- **源 DTL 升版路径**:v0.1→v0.3
- **源 DTL 升版类型**:元数据复核
- **核心要点**:同步父 BAS-013 升版至 v0.3 + 复核 §3.4 ChatAbuseGuard/Signal 落实现状(无新章节,仅复核)
...
| 源 DTL 真源 | RGS-DTL-013 v0.1 | RGS-DTL-013 0.3(具体修订见 §A.1) |
...
- 源 DTL 升版内容仅为 元数据复核(元数据 / 装饰性 / 追溯性追加)时,本节无新缺口继承。
...
- 源 DTL 修订历史条目:见 RGS-DTL-013 §修订历史表
