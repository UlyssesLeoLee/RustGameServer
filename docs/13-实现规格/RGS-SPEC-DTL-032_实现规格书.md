...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-032 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口（见 §A.3），待 RGS-DTL-032 具名 DD Review |
| 源详细设计 | RGS-DTL-032（本 DTL 今日未升版，SPEC v0.2 为前瞻性草案，见 §A.1） |
| 实现范围 | Agent 运行时与工具链 |
...
本规格把 RGS-DTL-032 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-032 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

按 DTL-032 的 agent 生命周期、工具调用、权限、超时、重试、健康和 telemetry contract 实现。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | services/agent；crates/agent-runtime；deploy/agent | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-032 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
- **源 DTL 升版路径**：**今日未升版**（`git log --since="2026-08-26 00:00" --until="2026-08-26 23:59" -- docs/**/RGS-DTL-032_*.md` 无 commit）
- **源 DTL 升版类型**：**前瞻性草案**（非"今日升版沉淀"）
- **核心要点**：源 DTL v0.2（2026-08-20）固化 L0 补偿闸门：JWT—载荷绑定、动作/货币/正数校验、intent/order 双幂等、玩家/全局日额度原子预留、重放拒绝审计及测试判定。本 SPEC v0.2 仅作元数据对齐，不在 §1~§8 引入新设计。
...
| 源 DTL 真源 | RGS-DTL-032 v0.1 | RGS-DTL-032 `0.2`（具体修订见 §A.1） |
...
- 源 DTL 修订历史条目：见 RGS-DTL-032 §修订历史表（本 DTL 今日未升版，引用最新一次历史升版）
...
| 0.2 | 2026-08-26 | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 对齐源 DTL-032 当前版本（`0.2`） + 头表 0.2 + 新增 §A v0.2 对齐说明；**不引入新设计**；**代签已允许**（per 2026-08-26 08:40 JST 偏好反转）；本 SPEC 为 17 份未升版 DTL 前瞻性 v0.2 草案之一（per RGS-OPEN-QA-2026-08-26-SPEC-v0.2 Q2） | §A（新增） |
