...
| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-SPEC-DTL-027 |
| 版本 | 0.2 |
| 状态 | 规格草案 + 已知缺口(见 §A.3)，待 RGS-DTL-027 具名 DD Review |
| 源详细设计 | RGS-DTL-027(本 DTL 今日未升版，SPEC v0.2 为前瞻性草案，见 §A.1) |
| 实现范围 | 客户端引擎适配层与 SDK |
...
本规格把 RGS-DTL-027 从详细设计转成可执行的实现清单，不替代源 DTL。若本规格与 RGS-DTL-027 不一致，以 DTL 评审变更为准；不得在代码中自行调和冲突。当前工作区没有对应实现源码，本文件不代表功能已完成。

实现引擎适配、C ABI/SDK、版本兼容、错误映射和同步；客户端不取得服务端权威。

## 2. 实现单元

| 类型 | 计划路径 | 要求 |
|---|---|---|
| 公共契约 | crates/client-core；crates/client-sdk；ffi | crate/service 依赖边界必须显式登记 |
...
- RGS-DTL-027 的审批/风险条件已满足；源 DTL 的 TBD 已有批准处置。
...
| 0.2 | 2026-08-26 | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 架构师(Ulysses（一人公司 12 角色 per DEC-008）) | 对齐源 DTL-027 当前版本(`0.2`) + 头表 0.2 + 新增 §A v0.2 对齐说明;**不引入新设计**;**代签已允许**(per 2026-08-26 08:40 JST 偏好反转);本 SPEC 为 17 份未升版 DTL 前瞻性 v0.2 草案之一(per RGS-OPEN-QA-2026-08-26-SPEC-v0.2 Q2) | §A(新增) |
...
- **源 DTL**:RGS-DTL-027
- **源 DTL 今日状态**:`0.2`(`2026-08-20`)
- **源 DTL 升版路径**:**今日未升版**(`git log --since="2026-08-26 00:00" --until="2026-08-26 23:59" -- docs/**/RGS-DTL-027_*.md` 无 commit)
- **源 DTL 升版类型**:**前瞻性草案**(非"今日升版沉淀")
- **核心要点**:源 DTL 末次升版 v0.2(2026-08-20)纳入 RGS-REQ-030-ADD1,将候选组件与生产后端解耦,定义 `ApprovedBackendProfile` 的 BOM/许可证/ADR 启用门禁、边缘 TTL 和回源契约;候选实现不得视为已批准默认值。v0.1(2026-08-17)初版制定,落实 `asset_db` 三表 DDL、清单查询协议格式、签名验证与分桶算法伪代码、DistributionBackend 接口与自托管对象存储/反向代理**候选**。本 SPEC 今日(2026-08-26)无新升版对齐,仅做元数据对齐 + §A 缺口列项。
...
| 源 DTL 真源 | RGS-DTL-027 v0.1 | RGS-DTL-027 `0.2`(具体修订见 §A.1) |
...
- 源 DTL 修订历史条目:见 RGS-DTL-027 §修订历史表(本 DTL 今日未升版,引用最新一次历史升版 v0.2 / 2026-08-20)
- 父 BAS 升版条目:见对应父 RGS-BAS-027 §修订历史表(本 DTL 对应父 BAS,本日是否升版需自审)
