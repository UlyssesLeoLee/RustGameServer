...
| 治理角色（行） | 7 | 管理域 Lead / 架构师 / SRE / DBA / 安全 / shared-platform / saga 召集人 |
...
| 任务 \ 角色 | 管理域 Lead | 架构师 | SRE | DBA | 安全 | shared-platform | saga 召集人 |
|---|---|---|---|---|---|---|---|
| 1. RBAC 权限（rbac_grant） | **A** | C | I | C | C | I | I |
| 2. 配置变更（config_update） | **A** | I | I |
| 3. 公告发布（announce） | **A** | C | I | C | C | I | I |
| 4. 审计链（audit_log） | **A** | C | I | C |
| 5. 道具发放（player.item_grant）| R | C | I | C | C | I | C |
| 6. 运营决策（policy） | **A** | C | I | C | C | I | C |

**矩阵解读**：
- 管理域 Lead 在 6 任务中 6 次 A（注册 / 登录 / 角色 / OCC 冲突），1 次 R（资产扣减 + 道具发放）
...
| 治理角色 | 真实责任人 | 兼任关系 |
|---|---|---|
| 管理域 Lead | Ulysses | 兼架构师 / Saga 召集人（per DEC-008 一人公司 12 角色治理基线） |
...
| 域 Lead | 签字 | 日期 | 备注 |
|---|---|---|---|
| 管理域 Lead（Ulysses per DEC-008）| _待 DDD Review 阶段补签_ | — | per RGS-SPEC-CROSS-011 DDD Review 模板 §3 字段级核对 |
