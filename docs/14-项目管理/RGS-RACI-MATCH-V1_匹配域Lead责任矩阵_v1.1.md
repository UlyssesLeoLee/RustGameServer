# RGS-RACI-MATCH-V1 匹配域 Lead 责任矩阵 v1.0（Match Domain Lead RACI v1.0）

**RGS-RACI-MATCH-V1**

| 项目 | 内容 |
|---|---|
| 文档编号 | RGS-RACI-MATCH-V1 |
...
| 适用范围 | match 域（房间生命周期 / 撮合 / tick 20Hz / 物理引擎 / 网络分区恢复 / Scene Actor） |
...
| 治理角色（行） | 7 | 匹配域 Lead / 架构师 / SRE / DBA / 安全 / shared-platform / saga 召集人 |
...
| 任务 \ 角色 | 匹配域 Lead | 架构师 | SRE | DBA | 安全 | shared-platform | saga 召集人 |
|---|---|---|---|---|---|---|---|
| 1. 房间创建（room_create） | **A** | C | I | C | C | I | I |
| 2. 撮合（matchmake） | **A** | I | I |
| 3. tick 驱动（tick 20Hz） | **A** | C | I | C | C | I | I |
| 4. 物理引擎 invariant | **A** | C | I | C |
| 5. 道具发放（player.item_grant）| R | C | I | C | C | I | C |
| 6. Scene Actor invalid | **A** | C | I | C | C | I | C |

**矩阵解读**：
- 匹配域 Lead 在 6 任务中 4 次 A（注册 / 登录 / 角色 / OCC 冲突），1 次 R（资产扣减 + 道具发放）
...
| 治理角色 | 真实责任人 | 兼任关系 |
|---|---|---|
| 匹配域 Lead | Ulysses | 兼架构师 / Saga 召集人（per DEC-008 一人公司 12 角色治理基线） |
...
| 域 Lead | 签字 | 日期 | 备注 |
|---|---|---|---|
| 匹配域 Lead（Ulysses per DEC-008）| _待 DDD Review 阶段补签_ | — | per RGS-SPEC-CROSS-011 DDD Review 模板 §3 字段级核对 |
