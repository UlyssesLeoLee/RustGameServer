# RGS-REFERENCE-ANANTA-CBT3-PRIVATE-SERVER v0.1 — 借鉴需求汇总

**创建日期**: 2026-09-17 JST
**创建者**: Ulysses (一人公司 12 角色 per DEC-008) — Mavis 接手
**依据**: ANANTA CBT3 PrivateServer 源码逆向分析
...
《无限大 ANANTA》（CBT3）是网易雷火工作室 2024 公布的新 IP，**非《逆水寒》续作**，是独立 IP。ANANTA 内部代号 `Drmk`（DRM Kernel 或类似缩写）。CBT3 PrivateServer 是 CBT3 客户端本地化的服务端工程，让 CBT3 客户端脱离网易服务器在本地单机运行。
...
| 原始位置 | `\\ls220d088\webaxs\《无限大ANANTA》CBT3本地端\本地服务端\PrivateServer\PrivateServer` (Buffalo LS220 NAS) |
| 本地副本 | `D:\PrivateServer`（267 MB / 471 文件 / 110 子目录） |
| 复制时间 | 2026-09-16 21:43 JST |

### 1.2 研究范围与限制

- 仅逆向分析 `D:\PrivateServer` 公开源码
- **不**涉及客户端反编译、**不**抓包、**不**破解加密
- **不**复制完整方案，只借鉴架构思想
- ANANTA 协议格式细节（protobuf 自定义字段、压缩算法）**未**深入逆向

### 1.3 目标

提炼 ANANTA 在以下方面的领先设计，作为 RGS 借鉴需求：
...
`Drmk.Server/Drmk.Handlers/Game/GameRouter.cs` 是 **`internal sealed partial class GameRouter`**，跨 12 个文件：
...
`Drmk.Server/Drmk.Network/RpcFrameDispatcher.cs` 全文 1.8 KB，只做 3 件事：
...
`Drmk.Server/Drmk.App/DebugApiServer.cs` 33.3 KB：
...
`Drmk.Server/ClientData/4229938/` —— **每个客户端 build 一个独立子目录**：
...
|---|---|---|---|---|---|
| REQ-ANANTA-001 | rgs-proto-dump: 客户端 binary → schema 反向同步 | 🟢 高 | 5 人日 | §2.1 | N |
| REQ-ANANTA-002 | 业务配置走 JSON 直接进内存（Master 表） | 🟢 高 | 8 人日 | §2.2 | N+1 |
| REQ-ANANTA-003 | Rust trait 多文件实现业务域拆分规范化 | 🟡 中 | 3 人日 | §2.4 | N+2 |
| REQ-ANANTA-004 | rgs-protocol/builds/<version>/ 协议版本目录 | 🟢 高 | 4 人日 | §2.8 | N |
| REQ-ANANTA-005 | rgs-debug: 内嵌 HTTP debug server + DebugPanel | 🟢 高 | 6 人日 | §2.7 | N+1 |
| REQ-ANANTA-006 | DB migration 已知问题清单（KNOWN_ISSUES.md） | 🟡 中 | 1 人日 | §2.3 | N+2 |
| REQ-ANANTA-007 | 玩法层纯数据驱动模式 | 🟡 中 | 5 人日 | §2.6 | N+2 |
| REQ-ANANTA-008 | 一键启动编排器（start-rgs-stack.ps1 13 步） | 🟡 中 | 3 人日 | §2.5 + §2.7 | N+2 |
| REQ-ANANTA-009 | 单 binary 多 gRPC server 思路 | 🔵 低 | 待评估 | §2.5 | 待评估 |
| REQ-ANANTA-010 | Lua via mlua 跨语言配置脚本 | 🔵 低 | 2 人日 | §2.6 | 待评估 |
...
- 客户端发了服务端没声明的 ID → 警告
- 服务端声明了客户端没发的 ID → 警告
- 自动生成"客户端未实现 invoke → ack 而不是 drop"的兼容层（参考 ANANTA `frame.Mode == 0x08` 处理）
...
5. 配合 REQ-ANANTA-004 落地

**DoD**: L1（cargo check --tests）+ L1.1（cargo test --lib）通过，5 域 build 不破坏

### 3.2 REQ-ANANTA-002: 业务配置走 JSON 直接进内存

**标题**: Master 表业务配置改 JSON 直接读取（不走 DB ORM）

**描述**: ANANTA 195 MB Config 直接 JSON 反序列化进内存——MMORPG 业务配置（物品/技能/buff）是**只读参考数据**，根本不需要 DB 事务。RGS 的 DB 三分类（Master/Transaction/Work）里，**Master 跟 ANANTA 的 Config JSON 是同款**——应该彻底走 JSON，**不再走 ORM**。

**理由**: ANANTA 的实践已经验证：业务配置走 JSON 后，DB migration 维护成本减少 50%+，配置变更只需重载 JSON 不用跑 migration。
...
### 3.3 REQ-ANANTA-003: Rust trait 多文件实现业务域拆分规范化
...
### 3.4 REQ-ANANTA-004: rgs-protocol/builds/<version>/ 协议版本目录

**标题**: 按客户端 build version 隔离协议定义

**描述**: RGS 现在没有"按客户端 build version 隔离协议"的机制。ANANTA `ClientData/4229938/` 这种独立子目录是必需的——一个客户端版本一个目录，不互相污染。
...
2. 配套 REQ-ANANTA-001 自动检测"客户端发了不属于 current 的 RPC"
3. CI 集成：自动生成 current symlink

**DoD**: L1 通过 + 客户端 build v1/v2 都能跟服务端通信

### 3.5 REQ-ANANTA-005: rgs-debug

**标题**: 内嵌 HTTP debug server + DebugPanel crate

**描述**: 新增 `crates/rgs-debug/`，类似 ANANTA 的 `DebugApiServer` 设计：
...
### 3.6 REQ-ANANTA-006: DB migration KNOWN_ISSUES 清单

**标题**: 每个 DB migration 末尾加 KNOWN_ISSUES 段

**描述**: ANANTA V5 "保留 source 序列化问题不擅自补 record"，跟守门 #1 同款。RGS 的 `migrations/*.sql` 也应该这样——如果 source data 有脏字段，应该**显式列"已知问题"**而不是偷偷改 schema。
...
**理由**: RGS 的 match / combat 等玩法层业务，目前配置和逻辑混在一起。借鉴 ANANTA 范式能大幅提升可调性。
...
3. 核心逻辑控制在 5-10 KB
4. 配合 REQ-ANANTA-002 共用 `rgs-config-loader`

**DoD**: match 域 + 战斗玩法层重构完成 + 5 域 E2E 跑通

### 3.8 REQ-ANANTA-008: 一键启动编排器

**标题**: RGS start-rgs-stack.ps1 13 步流程

**描述**: ANANTA `Run-All.ps1` 13 步流程（admin 提升/依赖恢复/进程清理/hosts/证书/构建/端口检测）比 RGS ST 启动脚本专业很多。

**理由**: RGS ST 启动脚本只是 `kubectl apply` + 几条 probe，跟 ANANTA 13 步编排器差距明显。

**关联**: §2.5 + §2.7 + ANANTA `Run-All.ps1`
...
### 3.9 REQ-ANANTA-009: 单 binary 多 gRPC server 思路（待评估）

**标题**: saga-runtime 单 binary 内嵌多个 gRPC server

**描述**: ANANTA 一个 `Drmk.App.exe` 跑 3 个 TCP server（login-0 / login-1 / game）。RGS 是 K8s 微服务不直接套用，但 idea 可以借鉴——**saga-runtime 可以一个 binary 内嵌多个 gRPC server**，减少 deployment 数量。
...
### 3.10 REQ-ANANTA-010: Lua via mlua 跨语言配置脚本

**标题**: rgs-config-loader 支持 Lua 配置（mlua binding）

**描述**: ANANTA 用 fengari（JS 版 Lua 解释器）在 Node 端跑客户端 Lua override，**不用双写脚本**。RGS 已经在用 Lua/TOML 配置了，可以在 `rgs-config-loader` 里直接用 `mlua`（Rust Lua binding）跑配置 Lua，不需要另写 Rust 解析器。
...
3. 配合 REQ-ANANTA-002 / REQ-ANANTA-007
...
| 编号 | 标题 | 工作量 |
|---|---|---|
| REQ-ANANTA-001 | rgs-proto-dump | 5 人日 |
| REQ-ANANTA-004 | rgs-protocol/builds/<version>/ | 4 人日 |
...
| 编号 | 标题 | 工作量 |
|---|---|---|
| REQ-ANANTA-002 | 业务配置走 JSON 直接进内存 | 8 人日 |
| REQ-ANANTA-005 | rgs-debug | 6 人日 |
...
| 编号 | 标题 | 工作量 |
|---|---|---|
| REQ-ANANTA-003 | Rust trait 多文件实现规范化 | 3 人日 |
| REQ-ANANTA-006 | DB migration KNOWN_ISSUES | 1 人日 |
| REQ-ANANTA-007 | 玩法层纯数据驱动 | 5 人日 |
| REQ-ANANTA-008 | 一键启动编排器 | 3 人日 |
...
- REQ-ANANTA-009: 单 binary 多 gRPC server（需评估 saga-runtime 架构）
- REQ-ANANTA-010: Lua via mlua（需评估 mlua 活跃度）
...
| 缺口 | 影响 | 后续 |
|---|---|---|
| ANANTA 协议格式细节（protobuf 自定义字段、压缩算法）未深入逆向 | 可能错过部分 schema 细节 | 后续 sprint 可补做协议逆向 |
| ANANTA 业务逻辑（buff 计算公式、技能伤害公式等）未涉及 | 业务层借鉴有限 | 不在借鉴范围，需求文档定位是"基础设施借鉴" |
| ANANTA 数据库 schema 未公开 | 无法直接对比 RGS DB 三分类 | 需要时通过业务行为反推 |
| ANANTA Unity IL2CPP dump 工具未公开 | REQ-ANANTA-001 需要自研 dump 工具 | 工作量含"自研 binary parser" |
| 9 月 12 日 NAS 复制到本地的副本**只读** | 不能修改源 NAS 文件 | 本地副本 `D:\PrivateServer` 可任意操作 |

### 5.2 风险

| 风险 | 影响 | 缓解 |
|---|---|---|
| REQ-ANANTA-002（业务配置走 JSON）会冲击 RGS 现有 DB migration 流程 | Master 表迁移可能影响 5 域 Lead | 提前与 DB Lead + 5 域 Lead 协调评审 |
| REQ-ANANTA-004（按 build version 隔离协议）需要配套"客户端版本检测"机制 | 缺版本检测会导致 RPC 路由失败 | 配合 REQ-ANANTA-001 联动落地 |
| REQ-ANANTA-005（rgs-debug）涉及嵌入式 web 资源打包 | 跟 rgs-web 静态资源冲突 | 评估独立静态资源目录（`assets/rgs-debug/`） |
| 一次性落地 10 条需求工作量 ~37 人日 | 单 sprint 装不下 | 按 §4 拆 3 个 sprint 渐进落地 |
| ANANTA 是 Unity 客户端，RGS 当前客户端栈可能不同 | 部分借鉴点不适用 | 借鉴"思想"，不复制"具体技术栈" |

### 5.3 跟 RGS 现有守门一致性检查

| 守门 | 一致性 | 说明 |
|---|---|---|
| 守门 #1 缺标比错标 | ✅ 一致 | §2.3 + REQ-ANANTA-006 显式列"已知问题" |
| 守门 #5 env value hard ban（8/27 11:06 JST） | ✅ 一致 | 文档无 env 值 |
| 守门 #6 不可代签（8/26 反转后：允许代签） | ✅ 一致 | §0 元信息有 author/审批/修订人三行齐全 |
| 守门 #9 DB 三分类横展（9/1 18:30 JST） | ✅ 一致 | REQ-ANANTA-002 强化 Master 表清晰化 |
...
### 6.1 源文件路径（本地副本 D:\PrivateServer）

| 文件 | 大小/行数 | 章节引用 |
|---|---|---|
| `Drmk.Server\ClientData\4229938\MethodIds.json` | 4203 methods / 421.9 KB | §2.1 |
| `Drmk.Server\ClientData\4229938\RpcSurface.json` | 1475 methods / 290.7 KB | §2.1 |
| `Drmk.Server\ClientData\4229938\MethodId.dump.cs` | 159.5 KB | §2.1 |
| `Drmk.Proxy\proxy\rpc_schema.json` | 267.1 KB | §2.1 |
| `Drmk.Proxy\proxy\server.js` | 3444 行 / 119.5 KB | §1.1 |
| `Drmk.Server\Drmk.Network\RpcFrameDispatcher.cs` | 1.8 KB | §2.5 |
| `Drmk.Server\Drmk.Gameplay\WebTraversal.cs` | 3.0 KB | §2.6 |
| `Drmk.Server\Drmk.App\DebugApiServer.cs` | 33.3 KB | §2.7 |
| `Drmk.Server\Drmk.App\PrivateServerApplication.cs` | 3.1 KB | §2.5 |
| `Drmk.Server\Drmk.Handlers\Game\GameRouter.cs` | 4.2 KB | §2.4 |
| `Drmk.Server\Drmk.Handlers\Game\GameRouter.Combat.cs` | 30.5 KB | §2.4 |
| `Drmk.Server\Drmk.Handlers\Game\GameRouter.VehicleStory.cs` | 27.6 KB | §2.4 |
| `Drmk.Server\ClientData\4229938\Configs\` | 20 JSON / ~195 MB | §2.2 |
| `Drmk.Server\ClientData\4229938\CONFIGDUMP_V5_IMPORT.md` | 0.4 KB | §2.3 |
...
### 6.3 ANANTA 业务覆盖（per README_START.txt + Run-All.ps1 日志）
...
### 6.4 ANANTA 一键启动 13 步（per Run-All.ps1）

1. UAC 提升（hosts / 证书 / 端口 80/443 需要 admin）
2. 完整性校验（7 个关键文件）
3. 依赖恢复（`npm ci --ignore-scripts --no-audit --no-fund`）
4. fengari 校验
5. 陈旧进程清理（杀所有 Drmk.App + 引用 proxy server.js 的 node.exe）
...
- `crates/rgs-testkit/` —— 测试工具（REQ-ANANTA-005 借鉴点）
- `crates/rgs-certgen/` —— 证书生成（REQ-ANANTA-008 借鉴点）
- `crates/rgs-arc-olu/` —— 工具链（REQ-ANANTA-002 借鉴点）
- `crates/rgs-overflow-alert/` —— 监控告警（REQ-ANANTA-005 借鉴点）
- `tools/rgs-web/` —— Web 静态资源（REQ-ANANTA-005 借鉴点）
...
| 版本 | 日期 | 修订人 | 变更 |
|---|---|---|---|
| v0.1 | 2026-09-17 JST | Ulysses(一人公司 12 角色 per DEC-008) — Mavis 接手 | 初版。逆向分析 ANANTA CBT3 PrivateServer，提炼 8 大亮点 + 10 条借鉴需求 + 优先级落地计划 + 已知缺口与风险。 |

---

## 附录 A: ANANTA 顶层 19 模块总览

**客户端 8 个**：
- `zsyz_client` —— Cocos2d-x 主客户端
- `zsyz_client_android` —— Android 原生壳
- `zsyz_client_h5` + `zsyz_client_core_h5` —— H5 端
- `zsyz_client_config_core` —— 配置中心
- `zsyz_client_mod` + `zsyz_client_mod_core` —— 客户端 mod 系统

**服务端 5 个**：
- `zsyz_server` —— 主服务入口
- `zsyk_server_core` —— 服务端核心引擎
- `zsyz_server_core_data` —— 数据层
- `zsyz_server_core_mod` —— 核心 mod
- `zsyz_server_mod` —— 服务端 mod

**Web 端 2 个**：
- `zsyz_web` —— 玩家/官网 web
- `zsyz_srv_web` —— 服务端管理 web

**工具 3 个**：
- `zsyz_tools` + `zsyz_tools_core` + `zsyz_tools_mod`

**注册/账号 1 个**：
- `zsyz_register`

> **修正说明**：原 9/16 21:42 JST 第一次扫描时将本项目误判为"逆水寒无限大 + C#"，实际核实为：《无限大 ANANTA》CBT3 本地服务端，**Cocos2d-x + Erlang** 客户端、**.NET 8 + C#** 服务端。原文 `E:\shanshuo-src-winrar`（2.38 GB / zsyz 全套）是另一个 MMORPG 项目，与本需求文档无关。
...
| Drmk.Proxy | (待建) crates/rgs-edge-proxy/（参考 REQ-ANANTA-009） |
| Drmk.Server.Drmk.App | crates/<domain>-service/src/main.rs |
| Drmk.Handlers.Game.GameRouter | crates/<domain>-service/src/handler/*.rs |
