...
| **0.2** | 2026-09-07 | 架构师（Mavis 接手代签 per DEC-008） | 8 维度增量升版：① 8 域扩展（per 9/6 闪烁之光兼容）② 9 域 mTLS 业务级断点续传（per 9/6 d270ab9 11 步 v3 + d15a0bb 3 NEW 域 k8s yaml）③ 派生约束守护 L15-L23（per 9/6 6c6839e cutover） |
...
|---|---|---|---|---|
| 1 | **8 域扩展** | §2.1 R001~R006 仅覆盖 player/economy 域 SDK 集成 | §2.1 增 R007~R012：scene/battle/network/account/sub8 5 NEW 域 SDK + rgs-asset-download 跨域断点续传 | a5235eb/95e67a6/1134cfd/57edbeb/b6b19b7/1dd9afc/3c79bca/42df673 |
| 2 | **9 域 mTLS 业务级断点续传** | §2.2 R020~R026 仅含 HTTP Range + MinIO，无 mTLS | §2.2 增 R027~R030：9 域 mTLS 客户端模拟器 v3 + 断点续传链路（per 9/6 d270ab9 11 步 v3） | d270ab9 / d15a0bb |
...
覆盖断点续传模块与既有 SDK 模块（`rgs-asset-update` / `rgs-version` / `rgs-network`）+ 后端 `DistributionBackend`（自托管 MinIO + 商业 CDN 可选）的跨模块集成场景，验证 SDK 内部模块边界与跨进程边界均无破坏。v0.2 新增 8 域 SDK 集成与 9 域 mTLS 业务级断点续传链路。
...
| **TST-IT-04-R007** | **TL-3** | **FR-CDN-040 (8 域扩展 per 9/6)** | **scene 域 asset bundle 断点续传** | scene 场景资源 1MB~100MB 范围，Range 命中 | scene 1k 资源 |
| **TST-IT-04-R008** | **TL-3** | **FR-CDN-040 (8 域扩展 per 9/6)** | **battle 域战斗资源断点续传** | battle 战斗资源 100MB~500MB 范围，Range 命中 | battle 500 资源 |
| **TST-IT-04-R009** | **TL-3** | **FR-CDN-040 (8 域扩展 per 9/6)** | **network 域协议包断点续传** | network 协议包 10MB~50MB 范围，Range 命中 | network 200 资源 |
| **TST-IT-04-R010** | **TL-3** | **FR-CDN-040 (8 域扩展 per 9/6)** | **account 域账号资源断点续传** | account 账号资源 1MB~10MB 范围，Range 命中 | account 100 资源 |
| **TST-IT-04-R011** | **TL-3** | **FR-CDN-040 (8 域扩展 per 9/6)** | **sub8 8 子系统断点续传** | sub8 8 子系统资源，Range 命中 | sub8 800 资源 |
| **TST-IT-04-R012** | **TL-3** | **FR-CDN-040 (跨域 batch per 9/1)** | **batch 域 asset bundle 断点续传** | batch 域 console/backend 静态资源断点续传 | batch 50 资源 |
...
| **TST-IT-04-R027** | **TL-4/E2E** | **FR-CDN-040 + 9 域 mTLS** | **9 域 mTLS 客户端模拟器 v3 走完 11 步断点续传（per 9/6 d270ab9）** | mTLS 客户端模拟器 v3 + Range 协议 + 断点续传 | 11 步客户端模拟器 v3 |
| **TST-IT-04-R028** | **TL-4** | **FR-CDN-040 + 9 域 mTLS** | **3 NEW 域 k8s yaml + mTLS cert 导入后断点续传（per 9/6 d15a0bb）** | scene / battle / account 三域 k8s yaml + mTLS cert 验证 | 3 NEW 域 yaml + 3 mTLS cert |
| **TST-IT-04-R029** | **TL-4** | **FR-CDN-040 + 9 域 mTLS** | **mTLS cert 轮换时断点续传保持（per L-CAND-006）** | 9 域 cert 轮换，Range 命中保持 | 9 域 × 30 天 cert |
| **TST-IT-04-R030** | **TL-4** | **FR-CDN-040 + 9 域 mTLS** | **mTLS 业务级 = saga 触达（per L19 cutover 派生约束）** | 断点续传链路走 mTLS 通道，与 saga 触达联动 | mTLS + saga trace_id |
...
| C027 (R027) | SDK ↔ 9 域 mTLS | 11 步客户端模拟器 v3 | 断点续传走 mTLS 通道，11 步全部通过（per 9/6 d270ab9）。 |
| C028 (R028) | SDK ↔ 3 NEW 域 k8s yaml | mTLS cert 导入 | scene / battle / account 三域 k8s yaml + mTLS cert 验证通过。 |
| C029 (R029) | SDK ↔ mTLS cert 轮换 | cert 轮换 | 9 域 cert 轮换后断点续传保持，0 中断。 |
| C030 (R030) | SDK ↔ mTLS + saga | saga 触达 | 断点续传与 saga 联动，trace_id 贯通。 |
...
| FR-CDN-040 | TST-IT-04-R006, R020, R023, R026, R040, R007~R012 (8 域扩展) |
...
| 9/6 8 域扩展 | TST-IT-04-R007~R012 |
| 9/6 d270ab9 9 域 mTLS | TST-IT-04-R027~R030 |
| L-CAND-006 cert 轮换 | TST-IT-04-R029 |
| L19 mTLS=saga 触达 | TST-IT-04-R030 |

## 5. 通过判定

- §2 全部 22 条用例 PASS（v0.1 16 条 + v0.2 6 条 R007~R012 + R027~R030）
- 商业 CDN 边缘 Range 命中率 ≥ 80%（NFR-CDN-114 门禁）
- 自托管 MinIO Range 行为 100% 符合 RFC 7233
- 跨 SDK 模块调用无循环依赖
- 协议版本协商被拒后断点续传链路完整
- 8 域 SDK 集成 5 NEW 域（scene/battle/network/account/sub8）+ batch 域断点续传全通过
- 9 域 mTLS 业务级断点续传 11 步客户端模拟器 v3 全通过
...
| L19 (mTLS 业务级 = saga 触达) | ✅ (TST-IT-04-R030 落档) | 6c6839e |
| L21 (跨工具链 gRPC Code 解析) | ✅ (per R030 mTLS + saga trace_id) | 6c6839e |
| L23 (4 层自动探针) | ✅ (断点续传 4 层探针) | 6c6839e |
| B3 (DDD Review 二审) | ⏳ (Mavis 自审 1 次停手, Ulysses 二审待) | AGENTS.md §3.x |

---

> 与 RGS-TST-IT-04 + RGS-TST-IT-04-ADD1 共存（v0.2 升版）。
