...
| 文档编号 | RGS-BAS-010 |
| 版本 | 0.1 |
| 父文档 | RGS-REQ-001第10章ARC-001〜017、RGS-REQ-006〜013第7章ARC-018〜026（本文档是既有全部架构方针的**横向归纳**，不新增独立需求；仅在§4发现真正的机制空白处给出补强设计） |
...
本文档与RGS-BAS-001〜009为**同层引用**关系（依README§3规则6三层位阶，本文档与其余BAS文档同属第3层）：本文档引用它们的既有设计作为归纳对象，**不得**用本文档的归纳单方面改变被引用文档的决定；若归纳过程中发现两者表述不一致，以被引用的原文档为准，并在本文档中标注需要原文档修订（见§9）。
...
| AC-PAT-002 | G-001〜G-013的补强设计逐项复核：均未引入新的独立组件/新的ARC级决定（G-011除外，已明确标注为建议） |
