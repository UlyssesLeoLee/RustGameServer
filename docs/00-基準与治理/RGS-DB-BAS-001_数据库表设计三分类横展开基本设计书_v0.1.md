...
| 角色 | 姓名 | 审批日 | 备注 |
|---|---|---|---|
| 制定（起草） | 架构师 | 2026-09-01 | 横展开三分类原则落地（per 2026-09-01 18:30 JST 拍板） |
| 评审（5 域 Lead） | player/economy/match/social/admin | — | 5 域各取代表表 → §3 |
...
| 物化 FK | 仅同库内弱引用；跨域不物化 |
| 例子 | `player_sessions` / `reservations` / `inbox` / `matchmaking_tickets` / `session_subscriptions` |
...
| 例子 | `transaction_ledger` / `sagas` / `moves` / `audit_log`（hash-chained）/ 6 域 `outbox` |
...
| 例子 | `players` / `player_characters` / `player_inventory` / `decks` / `accounts` / `matches` / `admin_users` / 6 LCM 表（realm_lifecycle_run 等） |
...
> **数据来源**：[`docs/15-IPA-DB表设计书/00-目录与全貌清单.md`](../15-IPA-DB表设计书/00-目录与全貌清单.md) + [`README.md`](../15-IPA-DB表设计书/README.md) §库/域 全局映射（**注**：该文件夹 18 个文件当前未 commit，git status `??`，引用关系见 §10）
...
| M-09 | `realm_lifecycle_run` | admin_db | Admin (LCM) | Type 2（按月分区，per FR-LCM-001）| [06-Admin域 §6.4](../15-IPA-DB表设计书/06-Admin域_admin_db.md#64-realm_lifecycle_run) |
| M-10 | `new_realm_plan` | admin_db | Admin (LCM) | Type 1 | [06-Admin域 §6.5](../15-IPA-DB表设计书/06-Admin域_admin_db.md#65-new_realm_plan) |
| M-11 | `split_plan` | admin_db | Admin (LCM) | Type 1 | [06-Admin域 §6.6](../15-IPA-DB表设计书/06-Admin域_admin_db.md#66-split_plan) |
| M-12 | `merge_conflict_rule_set_v2` | admin_db | Admin (LCM) | Type 1 + JSONB GIN 索引 | [06-Admin域 §6.7](../15-IPA-DB表设计书/06-Admin域_admin_db.md#67-merge_conflict_rule_set_v2) |
| M-13 | `retire_plan` | admin_db | Admin (LCM) | Type 1 | [06-Admin域 §6.8](../15-IPA-DB表设计书/06-Admin域_admin_db.md#68-retire_plan) |
| M-14 | `archive_policy` | admin_db | Admin (LCM) | Type 1 + partial index | [06-Admin域 §6.9](../15-IPA-DB表设计书/06-Admin域_admin_db.md#69-archive_policy) |
| M-15 | `cards` | card_db | Card (工具) | Type 1（catalog）| [08-Card域](../15-IPA-DB表设计书/08-Card域_card_db.md) |
| M-16 | `i18n_strings` | i18n_db | I18n (工具) | Type 1（多语言文案）| [09-I18n域](../15-IPA-DB表设计书/09-I18n域_i18n_db.md) |
| M-17 | `leaderboard_entries` | leaderboard_db | Leaderboard (工具) | Type 2（按 score 排名，hot zone 物理排序）| [10-Leaderboard域](../15-IPA-DB表设计书/10-Leaderboard域_leaderboard_db.md) |

**Master 横展开要点**：
- 5 核心业务域 + 1 工具域（Card）共 14 张核心 Master 表
- 6 张 LCM 表共置 admin_db（per FR-LCM-001，违反 ARC-008 "5 独立 DB 原则"已识别为 P0-01，见 [17-不合理设计 §P0-01](../15-IPA-DB表设计书/17-不合理设计识别与优化建议.md#p0-01lcm-6-表归-admin_db-破坏-arc-008-5-独立-db-原则)）
- 14 张主表全部有 `version` OCC 列或 `status` 状态机
- 跨域物化 FK 0 张（per RGS-BAS-007 §1.5）

## 3.2 Transaction 表（11 张 — append-only，事件流水）

| # | 物理表名 | 库 | 域 | 分区策略 | 设计書 |
|---|---|---|---|---|---|
| T-01 | `transaction_ledger` | economy_db | Economy | 按月 RANGE（建议 PH-2 实施，per P0-03）| [03-Economy域 §3.2](../15-IPA-DB表设计书/03-Economy域_economy_db.md#32-transaction_ledger) |
| T-02 | `sagas` | economy_db | Economy | 按月 RANGE（建议 PH-2 实施）| [03-Economy域 §3.3](../15-IPA-DB表设计书/03-Economy域_economy_db.md#33-sagas) |
| T-03 | `moves` | match_db | Match | 按月 RANGE（**必分区**，亿级）| [04-Match域 §4.4](../15-IPA-DB表设计书/04-Match域_match_db.md#44-moves) |
| T-04 | `audit_log` | admin_db | Admin | **按月 RANGE**（per RGS-BAS-007 §4 + P0-02，3 年 36 滚动）| [06-Admin域 §6.2](../15-IPA-DB表设计书/06-Admin域_admin_db.md#62-audit_log) |
| T-05 | `outbox` (player) | player_db | Player | 短期保留（per [13-Outbox跨域模板](../15-IPA-DB表设计书/13-Outbox跨域模板.md)）| [02-Player域 §2.6](../15-IPA-DB表设计书/02-Player域_player_db.md#26-outbox) |
| T-06 | `outbox` (economy) | economy_db | Economy | 同上（6 域共享模板）| [03-Economy域 §3.6](../15-IPA-DB表设计书/03-Economy域_economy_db.md#36-outbox) |
| T-07 | `outbox` (match) | match_db | Match | 同上 | [04-Match域 §4.7](../15-IPA-DB表设计书/04-Match域_match_db.md#47-outbox) |
| T-08 | `outbox` (social) | social_db | Social | 同上 | [05-Social域](../15-IPA-DB表设计书/05-Social域_social_db.md) |
| T-09 | `outbox` (admin) | admin_db | Admin | 同上 | [06-Admin域 §6.3](../15-IPA-DB表设计书/06-Admin域_admin_db.md#63-outbox) |
| T-10 | `outbox` (cluster_ops) | cluster_ops_db | ClusterOps | 同上 | [07-ClusterOps域](../15-IPA-DB表设计书/07-ClusterOps域_cluster_ops_db.md) |
| T-11 | `replay_metadata` | replay_db | Replay (工具) | append-only（罕见修改）| [11-Replay域](../15-IPA-DB表设计书/11-Replay域_replay_db.md) |

**Transaction 横展开要点**：
- 6 域 outbox **共享物理模板**（per [13-Outbox跨域模板](../15-IPA-DB表设计书/13-Outbox跨域模板.md)），保证跨域一致性
- `audit_log` 必须按月分区（per RGS-BAS-007 §4 + P0-02 修复项）
- `transaction_ledger` / `sagas` / `moves` 建议 PH-2 实施按月分区
...
| W-06 | `auctions` | economy_db | Economy | 中期（拍卖结束/超时清理，per 配置）| [03-Economy域 §3.7](../15-IPA-DB表设计书/03-Economy域_economy_db.md#37-auctions) |
| W-07 | `private_trades` | economy_db | Economy | 中期（私下交易完成/超时清理）| [03-Economy域 §3.8](../15-IPA-DB表设计书/03-Economy域_economy_db.md#38-private_trades) |
...
- 2 张 Economy 域的"中期"表（auctions / private_trades）虽然寿命长于典型 Work，但仍是"业务流程存在，结束后清理"模式，归 Work
- 跨域不物化 FK（per RGS-BAS-007 §1.5）

## 3.4 工具域补充表（6 张 — 横展开分类，引用各域 detail）

| # | 物理表名 | 库 | 域 | 分类 | 设计書 |
|---|---|---|---|---|---|
| X-01 | `card_series` | card_db | Card | Master | [08-Card域](../15-IPA-DB表设计书/08-Card域_card_db.md) |
| X-02 | `card_collections` | card_db | Card | Master（玩家收藏）| [08-Card域](../15-IPA-DB表设计书/08-Card域_card_db.md) |
| X-03 | `languages` | i18n_db | I18n | Master（语言清单）| [09-I18n域](../15-IPA-DB表设计书/09-I18n域_i18n_db.md) |
| X-04 | `cluster_nodes` | cluster_ops_db | ClusterOps | Master | [07-ClusterOps域](../15-IPA-DB表设计书/07-ClusterOps域_cluster_ops_db.md) |
| X-05 | `feature_flags` | cluster_ops_db | ClusterOps | Master（特性开关，Type 1）| [07-ClusterOps域](../15-IPA-DB表设计书/07-ClusterOps域_cluster_ops_db.md) |
| X-06 | `resume_token_index` | (asset_download) | AssetDownload | Master（断点续传索引）| [12-AssetDownload域](../15-IPA-DB表设计书/12-AssetDownload域_downloads_sqlite.md) |

> **合計**：17 Master + 11 Transaction + 8 Work + 6 工具补充 = **42 张表**（per [`00-目录与全貌清单`](../15-IPA-DB表设计书/00-目录与全貌清单.md) §3 全表清单）
...
| **Transaction** | `transaction_ledger` | `account_id` / `amount` / `tx_type` / `balance_after` / `saga_id` | 永久（按月分区，PH-2 实施）| [§3.2](../15-IPA-DB表设计书/03-Economy域_economy_db.md#32-transaction_ledger) |
...
| 分类 | 代表表 | 关键字段 | 生命周期 | 设计書 |
|---|---|---|---|---|
| **Work** | （暂无典型 Work 表 — guild 域业务以 Master 为主）| — | — | [05-Social域](../15-IPA-DB表设计书/05-Social域_social_db.md) |
...
> **已知缺口（per §9）**：Social 域当前 Master 为主，Work 表覆盖薄弱 — `invitations` / `pending_join_requests` 等可能 Work 表待 PH-2 业务确认。

## 4.5 Admin 域（admin_db）+ LCM 子模块

| 分类 | 代表表 | 关键字段 | 生命周期 | 设计書 |
|---|---|---|---|---|
| **Work** | （LCM 短期执行记录表 — 待 P0-01 修复后明确归类）| — | — | [06-Admin域 §6.x LCM 临时表](../15-IPA-DB表设计书/06-Admin域_admin_db.md) |
| **Transaction** | `audit_log` | `actor_id` / `action` / `target` / `prev_hash` / `hash` (hash chain) | 3 年（按月分区，per RGS-BAS-007 §4 + P0-02）| [§6.2](../15-IPA-DB表设计书/06-Admin域_admin_db.md#62-audit_log) |
| **Master** | `admin_users` | `username` / `password_hash` / `role` (4 角色) / `domain_scope` | 永久 | [§6.1](../15-IPA-DB表设计书/06-Admin域_admin_db.md#61-admin_users) |
...
| LCM (共置 admin_db) | admin_db | cluster-ops (LCM 子模块) | 6 | Master(6) | [06-Admin域 §6.4-§6.9](../15-IPA-DB表设计书/06-Admin域_admin_db.md) |

**横展开观察**：
- 工具域以 **Master 表为主**（配置/特性/参考数据）
- 唯一 Work 表 = `downloads` (SQLite 异构)
- 唯一 Transaction 表 = `replay_metadata`
- LCM 6 表是特殊情况——按 FR-LCM-001 共置 admin_db 打破 ARC-008 "5 独立 DB"原则
...
- 监控点 = `cleanup.last_run_at` + `cleanup.deleted_count`（per `db.*` 日志命名空间）
...
> 落实 Ulysses 2026-09-01 15:52 JST 决策（"各 BAS 文档功能章节加 log 设计且区分 debug/release 级"）+ RGS-BAS-007 v0.3 §1.1 模板（commit bf52973）。
...
|---|---|---|---|---|
| `db.bas_dbb_001.published` | RGS-DB-BAS-001 新版本经审批后正式发布（如本版本 v0.1 → v0.2 升版）| 1/季度 | release 必出（100% 强制全采样，per RGS-BAS-004 v0.3 §6.2）| 含 `version` / `effective_at` / `approver_id`；约 250B/条 × 1/季度 = 极低 |
...
> **缺标比错标安全**（per 2026-08-26 RGS-DTL-036 v1.4 hotfix 复盘）。本节显式列出"已知缺口"，避免假装覆盖。

| # | 缺口 | 影响 | 待确认 | 推荐阶段 |
|---|---|---|---|---|
| 9.1 | Social 域 Work 表覆盖薄弱（`invitations` / `pending_join_requests` 等可能 Work 表）| 业务流程临时数据可能落 Master 表导致数据膨胀 | Social Lead 业务确认 | PH-2 |
| 9.2 | Admin 域 LCM 短期执行记录表归类未明（Work vs Transaction）| P0-01 修复后需重新分类 | Admin Lead 业务确认 | PH-2 |
| 9.3 | Economy 域 `auctions` / `private_trades` 寿命长于典型 Work，归 Work 是否合理 | 拍卖/私下交易可能持续数天～数周，归 Work 可能过早清理 | Economy Lead 业务确认 | PH-2 |
| 9.4 | `transaction_ledger` / `sagas` / `moves` 分区策略仅"建议"，未实施 | 亿级数据后查询性能劣化 | SRE Lead 实施计划 | PH-3 |
| 9.5 | LCM 6 表共置 admin_db 打破 ARC-008 "5 独立 DB 原则" | P0-01 已识别，修复需 INC-002 升版 + DTL-042 同步 | 架构师拍板 | PH-2 |
| 9.6 | 15-IPA-DB表设计书/ 18 个文件 git status `??` 未 commit | 引用关系未正式入库 | 主会话 commit（代签，per 9/1 18:30 JST 原则）| 立即 |
| 9.7 | RGS-DB-BAS-001 v0.1 待 5 域 Lead 一审 | 三分类映射可能与各域业务实情有偏差 | 5 域 Lead 一审 | DDD Review v0.2 |
| 9.8 | 横展开三分类原则是否需要扩展到 Physis 物理引擎领域 | 跨项目一致性 | Ulysses 拍板 | 待定 |
...
RGS-BAS-007 (DB 設計標準 基本設計書 v0.3)
    ↓ 三分类横展开
RGS-DB-BAS-001 (本文档 v0.1) ← 你正在读
    ↓ 12 库 / 42 表 / 5 域 + 7 工具域
docs/15-IPA-DB表设计书/  (18 个 detail 文件, 当前未 commit)
    ├── 00-目录与全貌清单.md          (全表清单)
    ...
    └── 17-不合理设计识别与优化建议.md   (P0/P1/P2 优化点)
```
...
|---|---|---|---|---|---|
| 0.1 | 2026-09-01 | 架构师(Mavis 接手 agent per DEC-008) | 架构师(Mavis 接手 agent per DEC-008) | 初版制定。落实 2026-09-01 18:30 JST 拍板的"DB 表设计三分类横展开原则"（Work/Transaction/Master + IPA SEC 準拠），覆盖 RGS 全部 12 库 / 42 张表 / 5 核心业务域 + 7 工具域；§1 前言 / §2 三分类原则（判定矩阵 + 反例/正例）/ §3 跨域横展开总表（17 Master + 11 Transaction + 8 Work + 6 工具补充）/ §4 5 域代表表 / §5 7 工具域 / §6 跨域一致性（不物化 FK + OCC + append-only + cleanup job + 分区滚动）/ §7 IPA SEC 横展开（SEC-EM/PM/BOK）/ §8 本功能日志设计（5 列详尽版 + debug-only 守护，per RGS-BAS-007 v0.3 §1.1 模板）/ §9 已知缺口 8 项 / §10 引用关系图；引用 RGS-REQ-011 / RGS-BAS-007 v0.3 / `docs/15-IPA-DB表设计书/` 18 个 detail 文件（当前未 commit，per §9.6） | 全部 |

**修订人**：Ulysses（一人公司 12 角色 per DEC-008）— Mavis 接手
**审批**：架构师（Mavis 接手 agent per DEC-008）+ 自审 + 2026-09-01
**代签授权依据**：2026-08-27 19:39 / 20:56 / 21:59 JST Ulysses 三次强化"你可以代签" + 2026-09-01 18:30 JST "DB 三分类横展开原则"拍板
