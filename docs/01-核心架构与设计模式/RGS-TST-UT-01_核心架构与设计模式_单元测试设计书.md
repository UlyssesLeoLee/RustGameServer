...
| 制定日 | 2026-08-19 |
| 升版日 | 2026-08-27（v0.2→v0.3，5 份 DTL 增改对齐） |
| 制定者 | 架构师（v0.1 自动化产出 + v0.2 字段级深化 + v0.3 DTL 增改对齐） |
...
| **0.3** | 2026-08-27 | 架构师（Mavis 接手 agent per DEC-008） | **DTL 增改对齐**：对齐 5 份 DTL v0.2→v0.3 升版 + 新 ADR-0057 决定项。**新增 §8. v0.3 增量（per 5 份 DTL 增改）** 含 12 条新用例：DTL-001 §3.4 权威源分级 Tier-1/Tier-2（N001~N003）、DTL-001 §5.3/§7.2.1 死锁防止与背压八边界（N004~N005）、ADR-0057 Tier-1/Tier-2 + 一致性哈希同节点分片 + Reward Saga 语义（N006~N008）、DTL-024 §8 status 子命令部署时长基准 + DTL-022 §3.3 分片挂/合服（N009~N011）、DTL-001 §3.4 Tier-1 强一致不可逆资产字段清单（N012）。DTL-002/022/023 装饰性升版未引入新章节，沿用 v0.2 现有覆盖。**不重写 v0.2 既有章节**。 |
...
本文档为 V 模型中 **TL-1 单元试验**层级的设计书，对应主题 01 的 5 份详细设计书。本版本（0.3）相比 0.2 的核心升级：

- **DTL 增改对齐（v0.3 新增）**：5 份 DTL 自 2026-08-19 以来存在升版/实质变更——DTL-001 v0.5→v0.6（新增 §3.4 权威源分级 + §5.3 死锁防止与背压八边界 + §7.2.1 ARC-013 落实）、DTL-002 v0.2→v0.3（同步 BAS-002 v0.3 装饰性升版）、DTL-022 v0.2→v0.3（同步 BAS-022 v0.2 元数据层 + 追溯性表新增 §3.3 分片挂/合服引用）、DTL-023 v0.1→v0.2（同步 BAS-023 v0.2 装饰性升版）、DTL-024 v0.2→v0.3（复核确认 BAS-024 §9A 部署时长基准已落实于 §8 status 子命令退出码0 输出）。本版本新增 §8 增量章节，覆盖上述实质新增/复核点共 12 条新用例（N001~N012）
- **ADR-0057 决定项覆盖（v0.3 新增）**：本主题涉及的 10 份 ADR 之外，新增 RGS-ADR-0057（2026-08-25 Accepted：Tier-1/Tier-2 分级持久化 + 一致性哈希同节点分片 + Reward Saga 语义澄清），其决定项通过 §8 N006~N008 验证
- **既有 v0.2 字段级深化能力保留**：每条测试用例的"对应设计"列仍为"文档 ID + 章节 + 表号/图号/字段名"
- **既有 v0.2 ADR 决策验证能力保留**：§3.10 模块 K 覆盖本主题 10 份 ADR 的"实现位置 + 测试位置 + 守门位置"
- **既有 v0.2 TBD 处置能力保留**：§7 明确每条 TBD 在测试中的"按保守假设实施 / 标记为预留 / 待前置条件"三种处置方式
- **强化 V 模型对应**：§2.1 给出 V 字映射图，明确 UT 验证 DTL 哪些字段

注：v0.1→v0.2 的核心升级（字段级映射、ADR 决策验证、TBD 处置）仍见 v0.2 修订历史行。
...
| RGS-ADR-0057（v0.3 新增） | Tier-1/Tier-2 分级持久化 + 一致性哈希同节点分片 + Reward Saga 语义澄清（Accepted 2026-08-25） | §8 N006~N008 验证 |
...
| RGS-DTL-001 §3.4 权威源分级（v0.6 新增） | Tier-1 economy_db 强一致不可逆资产 + Tier-2 SceneActor 内存权威 | 字段清单 |
| RGS-DTL-001 §5.3 死锁防止与背压八边界（v0.6 新增） | 背压设置点八边界一览 | 全部 8 边界 |
...
| RGS-DTL-001 §7.2.1 ARC-013 死锁防止落实（v0.6 新增） | 跨 Actor 调用图证明 | 函数调用图 |
...
| RGS-DTL-022 §3.3 分片挂/合服（v0.3 追溯性新增） | 复用 BAS-002/020 既有流程（不越权展开） | 引用 + 守门 |
| RGS-DTL-023 §3 管道 | 4 前处理 + 4 后处理 | 函数签名 |
| RGS-DTL-024 §3-§8 集群 | 清单 DAG 状态机 | 全部 |
| RGS-DTL-024 §8 status 子命令（v0.3 复核确认） | 退出码0 打印 state/retry_count/updated_at | 字段输出 |
...
| DTL-001 §3.4 权威源分级（v0.6 新增） | Tier-1/Tier-2 字段清单 | TST-UT-01-N001~003, N012 | QA-001, QA-002 |
| DTL-001 §5.3 死锁防止与背压八边界（v0.6 新增） | 8 边界 | TST-UT-01-N004 | QA-001 |
| DTL-001 §7.2.1 ARC-013 落实（v0.6 新增） | 跨 Actor 调用图 | TST-UT-01-N004~005 | QA-001 |
...
| DTL-022 §3.3 分片挂/合服（v0.3 追溯性新增） | 引用 BAS-002/020 | TST-UT-01-N011 | QA-005（守门） |
...
| DTL-024 §8 status 子命令（v0.3 复核确认） | state/retry_count/updated_at 输出 | TST-UT-01-N009~010 | QA-001 |
| ADR-0001/0002/0007/0008/0015/0020/0022/0023/0026/0029 | 决定项 | TST-UT-01-K001~010 | QA-005 |
| ADR-0057 Tier-1/Tier-2 + 同节点分片 + Reward Saga（v0.3 新增） | 决定项 | TST-UT-01-N006~008 | QA-005 |
...
## 8. v0.3 增量（per 5 份 DTL 增改）

### 8.1 升版范围

本节对应 2026-08-19 至 2026-08-27 期间 5 份 DTL 的增改，按 DTL × 实质新增章节 × 新增 ADR 三个维度列出 v0.3 新增的 12 条测试用例（N001~N012）。

| DTL/ADR | 升版轨迹 | 实质新增章节 | 装饰性/元数据 | 触发源 |
|---|---|---|---|---|
| RGS-DTL-001 | v0.5 → v0.6 | §3.4 权威源分级 Tier-1/Tier-2、§5.3 死锁防止与背压八边界、§7.2.1 ARC-013 落实 | §14 追溯性追加 | BAS-001 v1.3/v1.4 升版 + ADR-0057 Accepted |
| RGS-DTL-002 | v0.2 → v0.3 | （无新章节） | 父 BAS-002 v0.3 装饰性同步 | RGS-IMPL-001 |
| RGS-DTL-022 | v0.2 → v0.3 | （无新章节，§3.3 分片挂/合服属 BAS-002/020 既有职责，本 DTL 不越权展开） | 追溯性表新增 BAS-022 §3.3 引用 | BAS-022 v0.2 升版 |
| RGS-DTL-023 | v0.1 → v0.2 | （无新章节） | 父 BAS-023 v0.2 装饰性同步 | 装饰性升版 |
| RGS-DTL-024 | v0.2 → v0.3 | §8 `status <run_id>` 退出码0 打印 state/retry_count/updated_at（v0.2 已落实，v0.3 复核确认） | 追溯表无新增行 | BAS-024 §9A 部署时长基准 P50 10min/P99 20min（NFR-DEP-003） |
| RGS-ADR-0057（v0.3 新增） | v0.1 Accepted 2026-08-25 | Tier-1/Tier-2 持久化 + 一致性哈希同节点分片 + Reward Saga 语义澄清 | — | 2026-08-25 Ulysses 一人公司 12 角色全签 |

### 8.2 新增测试用例

**N001~N003：DTL-001 §3.4 权威源分级 Tier-1/Tier-2（per BAS-001 §5.4.3 / ADR-0057 §2.1）**

| 用例 ID | 对应设计 | 字段级 / 函数签名 | 测试目的 | 覆盖类型 | 优先级 |
|---|---|---|---|---|---|
| TST-UT-01-N001 | DTL-001 §3.4 Tier-1 强一致不可逆资产 | economy_db ACID 事务 | 充值货币/抽卡/贵重物品销毁的同拍落库 | N | ◎ |
| TST-UT-01-N002 | DTL-001 §3.4 Tier-2 最终一致过程态 | SceneActor 内存权威 + 周期 Checkpoint (30s per ISS-010) | 坐标/技能冷却/任务计数/临时 Buff 的 Checkpoint 落盘 | N | ◎ |
| TST-UT-01-N003 | DTL-001 §3.4 重连恢复场景 | 旧 epoch 失效 + 新 epoch 签发（per BAS-001 §4.1.3） | 异步落库缺失窗口期的重连恢复（Tier-1 不允许此场景） | A | ◎ |

**N004~N005：DTL-001 §5.3 / §7.2.1 死锁防止与背压八边界（per BAS-001 v1.3 §7.2.1）**

| 用例 ID | 对应设计 | 字段级 / 函数签名 | 测试目的 | 覆盖类型 | 优先级 |
|---|---|---|---|---|---|
| TST-UT-01-N004 | DTL-001 §7.2.1 ARC-013 死锁防止 | 跨 Actor 调用图为有向无环图（DAG） | A→B→A 同步循环检测 | A | ◎ |
| TST-UT-01-N005 | DTL-001 §5.3 背压八边界一览 | 8 边界：Out1/Out2/Out3/Checkpoint/Snapshot/Replay/Reconnect/Restart | 八边界调用不阻塞 tick 主循环 | P | ◎ |

**N006~N008：ADR-0057 决定项（v0.3 新增）**

| 用例 ID | 对应 ADR | 决定项 | 实现位置（DTL） | 测试位置 | 守门位置 | 优先级 |
|---|---|---|---|---|---|---|
| TST-UT-01-N006 | RGS-ADR-0057 §2.1 | Tier-1 强一致不可逆资产同拍 ACID | DTL-001 §3.4 + DTL-001 §2.2 economy_db 事务 | TST-UT-01-N001 + IT-01 | 静态扫描：tier1 字段写入不得跨事务 | ◎ |
| TST-UT-01-N007 | RGS-ADR-0057 §2.2 | 一致性哈希同节点分片（player-service 与 economy-service 同节点部署） | DTL-001 §5.1 SceneActor 部署拓扑 + DTL-022 §3.1 shard_router | TST-UT-01-N007 + D001~006 | deployment check：colocation 标签 | ○ |
| TST-UT-01-N008 | RGS-ADR-0057 §2.3 | Reward Saga 不可逆事件 forward-only（不做撤销补偿） | DTL-001 §10.3 Saga 状态机 + DTL-100 §3.3 | TST-UT-01-N008 + F005~006 | lint: reward_saga_no_compensation | ◎ |

**N009~N011：DTL-024 §8 / DTL-022 §3.3（v0.3 复核 + 追溯性新增）**

| 用例 ID | 对应设计 | 字段级 / 函数签名 | 测试目的 | 覆盖类型 | 优先级 |
|---|---|---|---|---|---|
| TST-UT-01-N009 | DTL-024 §8 `status <run_id>` 退出码0 | stdout 含 `state`/`retry_count`/`updated_at` 字段 | CLI 输出契约 | N | ◎ |
| TST-UT-01-N010 | BAS-024 §9A 部署时长基准（NFR-DEP-003） | P50 ≤ 10 分钟 / P99 ≤ 20 分钟 | 度量数据源 = §8 `status` 输出时间戳序列（实测校准留 PH-4） | B | ○ |
| TST-UT-01-N011 | DTL-022 §3.3 分片挂/合服 | 引用 BAS-002/020 既有挂载/合服流程（不越权） | 守门：DTL-022 不重复定义挂载/合服协议 | A | △ |

**N012：DTL-001 §3.4 Tier-1 强一致不可逆资产字段清单**

| 用例 ID | 对应设计 | 字段级 / 函数签名 | 测试目的 | 覆盖类型 | 优先级 |
|---|---|---|---|---|---|
| TST-UT-01-N012 | DTL-001 §3.4 Tier-1 资产清单（per ADR-0057 §2.1） | wallets.balance / inventory_items.quantity / transaction_ledger 不可变追加 | 4 类不可逆资产字段被识别为 Tier-1 | N | ◎ |

### 8.3 装饰性升版 DTL 处置说明

DTL-002 / DTL-022 / DTL-023 三份装饰性/元数据升版未引入新章节，§2.2 字段映射表与 §4 追溯性矩阵已分别追加"装饰性/元数据"行明确归属；UT 用例不新增（沿用 v0.2 既有覆盖即可）。这是因为装饰性升版是"父 BAS 升版但本 DTL 不重写"，§2.2 已为这种治理状态预留了"v0.3 追溯性新增"标记行。

### 8.4 v0.3 增量不覆盖项（已知缺口 / 显式列出）

| 缺口 | 原因 | 处置 |
|---|---|---|
| TBD-CAP-001（T3 多区域方案校准值） | DTL-022 §3.3 分片挂/合服触发 T3 多区域评审时另走 BAS-017§2.3，本 UT 不越权 | 沿用 v0.2 既有"8 shard"保守假设 |
| TBD-DEP-001（Schema 校验实现语言） | DTL-024 §3 JSON Schema 校验实现代码留实施阶段 | 沿用 v0.2 既有"任意实现" |
| ADR-0057 §2.2 resharding/handoff | ADR-0057 修订历史明示"标注为 TBD" | 暂不补 UT，等 DTL/BAS 后续升版给出具体设计再补 |
| BAS-024 §9A 实测校准 | 部署时长基准声明"须 PH-4 实测校准" | TST-UT-01-N010 标记为"实测校准留 PH-4"，不设门禁 |

---

> 本文档为 RGS-TST 系列主题 01 单元测试设计书（**字段级深化版 v0.3** —— 在 v0.2 字段级深化基础上，对齐 5 份 DTL v0.2→v0.3 增改 + 新增 ADR-0057 决定项）。其他主题（02-07）见对应 `RGS-TST-UT-02〜07_*.md`。
