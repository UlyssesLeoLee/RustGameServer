...
| 0.3 | 2026-08-25 | 架构师（Ulysses（一人公司 12 角色 per DEC-008））| — | **同步父 BAS-022 升版至 v0.2**（1 次升版，BAS-022 v0.2 设计缺口补齐：§2.3 三类组件扩容手段对应表 / §3.3 分片新增/下线流程复用说明 FR-CAP-012 / §5 全容量级别可用性声明 FR-CAP-030）: 本 DTL 是父 BAS 的详细化（per DTL 头部"不改变任何既有决定"），父 BAS v0.2 升版为既有设计的缺口补齐与判定规则强化，DTL-022 既有章节对 §2.3/§5 升版内容已通过 §2 `validate_scaling_plan` 与 §5/§6 `target_shards`+`scope_shard_ids` 协议落实;对 §3.3（分片新增/下线复用 BAS-002/020 既有挂载/合服流程）属"不越权设计"原则覆盖范围,本 DTL 不展开具体协议,**仅在追溯性表新增一行明确归属**（新增 RGS-BAS-022§3.3 → 不在本 DTL 展开）。**正文本不重写**（per `RGS-DOCS-HEALTH-2026-08-25` §0 第 4 行"治理状态, 非文档缺陷, agent 不可代签" + 反馈单 §4 要求 1 "不预填任何 ✅, 不代签"）。 审批留空，待 Ulysses 在 review 时签发。 | 追溯性（新增 §3.3 引用行）；元数据层 |
...
| RGS-BAS-022§3.3 分片新增/下线流程复用说明（FR-CAP-012，补齐设计缺口）| 不在本DTL展开（归属RGS-BAS-002 挂载脚手架 / RGS-BAS-020§4 合服分服流程）|
