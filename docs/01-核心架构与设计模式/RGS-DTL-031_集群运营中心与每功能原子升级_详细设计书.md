...
本节是 v0.3 升版新增的"父 BAS→DTL 落实对账"小节。**本批次父 BAS-031 实质未升版**：父 RGS-BAS-031 末次变更为 2026-08-21 commit `e908471`（仅装饰性修订：`admin_db` 锚点大小写归一、§9.1 联动点指向 §3 占位改为指向 DTL-031、结尾段加入 DTL v0.1 草案已形成说明），**版本号仍为 0.1**，修订历史仅 1 行（v0.1 初版）。本节"父 BAS 升版内容"指 BAS-031 自 v0.1 初版制定至本批复核期间的**全部内容**（含 `e908471` 的装饰性修订），逐章核对 DTL-031 v0.2 落实情况。本表登记 BAS-031 §章节 → DTL-031 哪一节提供核对依据 / 排除声明，不重复 BAS-031 原文表。
...
- **实质重写判定**：DTL-031 v0.2 全文 12 章均已在 v0.1 草案 + v0.2 同步 IMPL-001 两次升版中落实父 BAS-031 v0.1（含 `e908471` 装饰性修订）全部 11 章内容，**本批次 v0.3 升版不需要正文重写**，仅做元数据层对账（与 DTL-007/016/018/020/021/023/038 同批次 10 份轻量升版模板一致，per `acc7e7a` commit message "DTL 升版语义: DTL 版本对齐到 BAS 当前版本（同步追溯）"，但因 BAS-031 实质未升版，本版本号 0.2→0.3 仅用于登记复核对账动作本身）。
