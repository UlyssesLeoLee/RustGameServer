...
| TBD-PAT-002 | RSK-PAT-001建议的CI校验此前从未落地实施（本文档归纳范围滞后11份文档达一整个开发阶段才被发现，正是该风险的真实发生）。具体校验脚本（检测跨文档章节引用失效、检测BAS-010归纳范围滞后）的实现方式需SRE/架构师评审，纳入RGS-BAS-009§4既有CI治理框架，而非另建独立流水线 | PH-1前，与RGS-BAS-009§4同批 |
