...
| 详细设计书 | **代码级如何实现（代码级HOW）**：函数签名、物理数据类型／索引／约束（DDL）、字节级线路格式 | 函数原型、DDL、字节布局 | RGS-IFS-001、各业务域 DTL 的相应物理 DDL 章节 |
...
| 各业务域详细设计书（物理 DDL 章节） | 分域数据库详细设计 | 本文档§5给出逻辑ER图（含属性）与UML类图；各业务域 DTL 给出相应物理DDL（数据类型・索引・约束・分区策略） |
...
| RGS-OPS-001（已制定） | 运维手册 | 已制定，见 RGS-OPS-001；以手册演练证据确认本文档§7.1可用性设计与§7.4可观测性设计目标 |
...
| ARC-013 | 背压设置位置与死锁防止 | §3.3、§4.2.4、§7.2、§7.2.1 |
...
> 本节为**逻辑（论理）级**设计：实体、属性、关系、基数、以及以UML表达的聚合边界。**不含**物理数据类型精度、索引、约束、分区策略——这些由各业务域 DTL 的相应物理 DDL 章节在详细设计阶段确定。属性的"类型"栏使用逻辑类型（字符串／整数／长整数／布尔／时刻／JSON），非PostgreSQL物理类型。
...
### 5.4.3 权威源分级：Tier-1（DB权威）与Tier-2（Actor权威）（per RGS-ADR-0057 §2.1）

SceneActor（§4.2.1）持有的角色状态字段，按权威源分两级，不因统一持有于同一Actor结构体而混同其持久化语义：

- **Tier-1字段（强一致不可逆资产：`Wallet.balance`、交易结果、抽卡结果、贵重物品销毁等，即本节`economy_db`所辖字段）**：`economy_db`为**权威源**，SceneActor侧持有的仅为**读缓存**（用于tick内高频读取，避免每次读取都跨服务往返）。写路径走既有§4.5.2确定请求（`Out2`异步调用economy-service），SceneActor发起请求后**不阻塞当前tick**（不改变§4.2.2 tick循环的阶段预算与NFR-PE-002 25ms总预算），但在收到`economy_db`提交成功的回执之前，**不得**向客户端下发该操作成功的确认。`economy_db`本身崩溃恢复时Tier-1字段天然完整，不依赖任何Checkpoint机制。
- **Tier-2字段（最终一致过程态：坐标、技能冷却、任务计数、临时Buff等，不落于本节`economy_db`，而是SceneActor内存独有）**：SceneActor内存为**权威源**，DB仅作周期性Checkpoint（暂定≤30秒，per附件D ISS-010探讨中，RPO上界为30秒）。SceneActor崩溃后从最近一次Checkpoint恢复，可能丢失至多一个Checkpoint周期内的Tier-2状态变化，这是既定可接受代价。

两者的区别落在"**权威源在哪**"（DB权威 vs Actor权威），而非"写入时机是否落在同一tick"——写路径均为异步、均不阻塞tick，与§4.2.2 tick循环结构、CON-007、ARC-007三项既有约束零冲突。
...
依据需求定义书DR-007〜015，全部承载永久事实（DR-002）且可能并发更新的表，逻辑结构须遵循以下范式（物理字段类型见各业务域 DTL 的相应物理 DDL 章节）：
...
### 7.2.1 背压设置点一览与死锁防止证明（ARC-013完整落地）

**背压设置点一览**：ARC-013要求"客户端连接、网关、场景mailbox、gRPC、事件消费者、数据库连接池、工作流Activity、日志路径"八处边界均须设置同时执行数上限・队列上限・超时・速率限制・熔断器・负载卸除。各边界在本文档的既有落地位置汇总如下（本表**不引入新机制**，仅做一次性收口，防止详细设计阶段遗漏边界）：

| 背压边界 | 落地位置 | 上限类型 |
|---|---|---|
| 客户端连接 | §4.1.1限流模块（FR-GW-006） | 每连接输入速率上限 |
| 网关 | §4.1.4输入路由步骤2（速率限制）、§3.2 HPA（连接数扩缩仍有上限） | 速率上限＋连接数上限 |
| 场景mailbox | §4.2.1有界Mailbox（ARC-001） | 队列深度上限 |
| gRPC（东西向调用） | §3.3"每条东西向连接必须在客户端侧设置连接池上限与超时" | 连接池上限＋超时 |
| 事件消费者 | §4.7.1分发器失败重试为"下轮重试"而非无界重试；消费者侧幂等吸收（不在本轮新增，消费者并发度上限属详细设计） | 重试节奏有界 |
| 数据库连接池 | §7.2"各服务对PostgreSQL的连接池须设上限" | 连接池上限 |
| 工作流Activity | §4.7.2"Activity级重试"（ARC-011"Activity的要求：幂等・可重试・超时有限"） | 重试次数＋超时 |
| 日志路径 | RGS-BAS-004§6采样设计（错误路径强制全量，正常路径限速采样，避免日志管道成为无界队列） | 采样率上限 |

**死锁防止的具体证明**：ARC-013"不得在Actor间构成同步请求应答的循环；若确需请求应答，须使用优先级不同的独立通道，并在设计时证明不会形成循环"，在本系统既有调用图上验证如下。

| 调用方向 | 调用性质 | 是否等待应答 |
|---|---|---|
| 网关 → 运行时 | §4.1.4，转发后不等待运行时处理完成才响应客户端 | 否（fire-and-forward，mailbox满则直接拒绝，不阻塞） |
| 运行时 → 经济服务 | §4.5.2确定请求，异步任务发起，**不阻塞**tick循环（ARC-007） | 否（tick继续推进，结果在下一tick或之后tick反映） |
| 经济服务 → 运行时 | 不存在。经济服务不主动调用运行时（§6.3接口图`EconomyService`无任何指向`RuntimeCaller`的方法） | 不适用 |
| 网关 → 玩家服务 | §4.1.2鉴权，网关侧在收到应答前停留在"未完成鉴权"状态，但该等待**不构成循环**（玩家服务不反向调用网关） | 是（单向请求-应答，非循环） |
| 业务服务 → 事件基础设施 | §4.7.1，仅写入outbox后由独立分发器异步发布，业务服务事务提交后立即返回，不等待事件被消费 | 否 |
| 事件基础设施 → 消费者（含工作流） | §8/§4.7.2，消费者处理后不产生指向发布方的同步应答（ARC-010"仅用于已发生事实的异步传播"） | 否 |

**判定结论**：全系统东西向调用图中，唯一的"等待应答"边（网关→玩家服务、运行时→经济服务的调用发起方视角）**均不存在反向的同步等待边**——玩家服务与经济服务均不会在处理请求过程中反过来同步等待发起方应答，因此调用图不存在环。这满足ARC-013"设计时证明不会形成循环"的要求：证明方法为对§3.3、§4.1、§4.5、§4.7、§6.3所列全部东西向调用边逐一核查其反向是否存在同步等待边，结果为否。**若详细设计阶段新增任何服务间同步调用**，须重新执行本表的核查并更新本节，防止调用图随功能增多而意外产生环——这是ARC-013在系统级设计文档中留下的可复用检查方法，而非仅执行一次的证明。
...
| 7 | 数据库物理设计（数据类型精度、索引、约束、分区、DDL）——实体与逻辑属性已在§5给出，本项仅剩物理化 | §5 | — | PH-2（相应业务域 DTL 评审） |
...
| ARC-013 | 背压与死锁防止 | §3.3、§4.2.4、§7.2、§7.2.1（死锁防止的具体证明） | ✓ |
