...
-- DB：match_db（match 域 — 房间 / 匹配 / tick）
-- 状态：🔴 NO-GO 占位（per RGS-PLAN-001 v0.7 §3.3 + RGS-ENV-001 v0.2 §6 12 类签字栏）
-- 描述：初始 schema：rooms / match_tickets / room_state_history 表（按时间分区）
--
...
--   - match 域 Lead：待具名（per DEC-005 独立配置）
--   - SRE：待具名（per RGS-EXEC-001 v0.2 §4.4 所有者背书）

-- ===== NO-GO 占位：以下无 DDL =====
-- 实际 schema 在 NO-GO 解除后由 DBA + match 域 Lead 联合编写
