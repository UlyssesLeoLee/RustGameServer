-- 0002_coc_audit_log_placeholder.sql
-- DB：admin_db（admin 域 — COC 集群运营中心 + 审计 + 配载）
-- 状态：🔴 NO-GO 占位（per RGS-PLAN-001 v0.7 §3.3 + RGS-ENV-001 v0.2 §6 12 类签字栏）
-- 描述：COC 审计日志：audit_log 表（PGAudit + 触发器自动写入，per ARC-051）
--
...
--   - admin 域 Lead：待具名（per DEC-005 独立配置）
--   - SRE：待具名（per RGS-EXEC-001 v0.2 §4.4 所有者背书）

-- ===== NO-GO 占位：以下无 DDL =====
-- 实际 schema 在 NO-GO 解除后由 DBA + admin 域 Lead 联合编写
