...
本节覆盖**文档级 / 阶段门禁**事件——本文档属于设计文档，本身不产生业务流量，但 ARC-047 FEC 层引入运行时（上线后）会产生 FEC 编码/解码/丢包率/冗余率调整等事件，下游详细设计书（RGS-DTL-038-防丢包）将完整定义。本节先标记**安全审计相关**的高优先级事件（per RGS-BAS-006 v0.4 §6.2 强制全采样白名单精神）。
...
| 决策表 | 详见 §5.5 |
| 反馈协议 | 复用 ARC-003 Datagram 路径的应用层控制消息（FEC 控制帧独立于数据帧，详见 RGS-DTL-038-防丢包 §2） |
...
| AC | 验收标准 | 落实位置 |
|---|---|---|
| AC-NET-001 | FEC 在既定丢包率区间内可在不触发重传的前提下恢复约定比例的 Datagram，且解码延迟满足 NFR-NET-001 | §5.5 决策表 + RGS-DTL-038-防丢包 §5 |
| AC-NET-002 | FEC 冗余开销在既定丢包率区间内落在 NFR-NET-002 预算内，且随丢包率下降而降低 | §5.5（0.5% 以下关闭 FEC） |
| AC-NET-003 | 重连回归试验：新增 FEC 层后，重连恢复时延不劣于 NFR-NET-003 基线 | §3.1 G-NET-004 + RGS-DTL-038-防丢包 §6 |
| AC-NET-004 | 必达事件（IF-001-2）传输行为不因本文档变更（FR-NET-006） | §4.2 矩阵 Stream 行 + RGS-DTL-038-防丢包 §6 |
...
| TBD ID | 内容 | 来源 | 期限 | 负责人 |
|---|---|---|---|---|
| TBD-NET-001 | FEC 编解码库/实现方式选型（自研 XOR parity vs. RS(n, k>1) 扩展 vs. 第三方 crate），须经 ARC-014 判定 | RGS-REQ-038 §11 | 详细设计阶段前（RGS-DTL-038-防丢包 §4 对接前） | 架构师＋实时负责人 |
...
| v0.2 | 评审意见吸收 + 详细设计书（RGS-DTL-038-防丢包）落地 + G-CODE-06 Rust 1.98 stable 可用性复核 | 详细设计评审通过 + RSK-NET-001 复核结果 |
