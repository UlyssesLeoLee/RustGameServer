...
| 0.2 | 2026-08-25 | 架构师（Ulysses（一人公司 12 角色 per DEC-008））| — | 同步父 BAS-004 升版至 v0.2 + 补追溯性表 AC-LOG-001〜005 验收标准与本文档章节的映射（BAS-004 v0.2 §12 追溯性表新增的 AC 条目此前未在本文档追溯性表中体现，本次仅追加映射行，不改既有章节内容） | 追溯性 |
...
6. [TBD-LOG-004：scene_id高基数分桶方案提案](#6-tbd-log-004sceneid高基数分桶方案提案)
...
| AC-LOG-001（trace_id全链路检索，父 BAS-004 v0.2 §12 追溯性表新增）| §2.1（基础字段由 SDK 自动注入，业务代码不可覆盖）+ §4（JSON 线格式中 `trace_id`/`span_id` 在上下文不可得时省略字段而非置空，避免下游检索按空字符串误匹配）|
| AC-LOG-002（禁止字段人工抽查未明文出现，父 BAS-004 v0.2 §12 追溯性表新增）| §3.1（黑名单拦截主算法：命中即替换 `[REDACTED]`）+ §3.2（分类脱敏规则：IP 掩码/邮箱手机哈希化/兜底丢弃）|
| AC-LOG-003（脚手架生成新服务无需手工编码即符合规范，父 BAS-004 v0.2 §12 追溯性表新增）| §2.1 / §2.2 / §2.3（统一埋点 SDK 的日志 / Span / 指标三组 trait 即脚手架产出物契约，业务代码只通过该 trait 调用，不导出 `tracing`/`log` 原始宏）|
| AC-LOG-004（错误路径与 GM 指令在采样率<100%下仍 100% 记录，父 BAS-004 v0.2 §12 追溯性表新增）| §5 `should_force_full_capture` 判定算法（四类条件：ERROR 级别 / AdminService 方法 / 高危操作 / 降级与背压拒绝，任一命中即忽略 `sample_ratio` 强制全采集）|
| AC-LOG-005（负载试验下存储/索引成本不突破运维负荷上限，父 BAS-004 v0.2 §12 追溯性表新增）| §2.3 指标 API（`labels` value 侧类型固定 `&str`，接口层防止任意字符串拼接成高基数标签）+ §6 `scene_id` 分桶方案（场景类型×分片 ID 组合维度替代 scene_id 本身，逐实例诊断改走慢 tick 日志+trace 检索）|
