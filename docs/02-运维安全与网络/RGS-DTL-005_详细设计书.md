...
| 0.2 | 2026-08-17 | 架构师 | — | 负责人指示"开子代理完成剩余的"（技术选型TBD收尾）。新增§7解决TBD-PLG-001（沙箱脚本引擎最终选型：Rhai，纯Rust MIT许可，语言层面天然沙箱、无FFI边界、内置资源限制API）。原§7覆盖范围章节顺延为§8并更新内容 | §1.2、§7（新增）、原§7→§8 |
| 0.2（同步BAS标记） | 2026-08-25 | 架构师（Ulysses（一人公司 12 角色 per DEC-008）） | — | 同步父 BAS-005 升版至 v0.2 + 补 AC 追溯性条目：父 BAS-005 v0.2 修订内容为"§11 追溯性表补齐 AC-PLG-001〜004 验收标准与设计章节的映射"（此前 BAS 追溯性表仅覆盖 ARC/FR/NFR，遗漏 AC 条目）。本 DTL 末尾"追溯性"表加 4 行 AC 映射（AC-PLG-001→§4+§5.1、AC-PLG-002→§3.1+§3.2+§6、AC-PLG-003→§2+§4+§5.1、AC-PLG-004→§2+§4），不引入新设计、不重写任何既有决定、不改动审批栏（不代签）。本质：DTL 文档追溯性对账 BAS-005 v0.2 §11，**不构成新 DTL 版本号**（版本号仍为 0.2），仅在修订历史中加此行作为同步标记 | 末尾"追溯性"表（加 4 行） |
...
7. [沙箱脚本引擎选型（TBD-PLG-001）](#7-沙箱脚本引擎选型tbd-plg-001最终决定)
...
- 沙箱脚本引擎最终选型（TBD-PLG-001）已于v0.2在§7给出最终决定（Rhai），不再是本文档遗留缺口。
...
## 7. 沙箱脚本引擎选型（TBD-PLG-001，最终决定）

选型为**Rhai**（纯Rust实现的可嵌入脚本引擎，MIT许可）。理由：

- **语言层面天然沙箱**：Rhai本身不提供文件系统/网络/进程访问的内置API，宿主需显式注册函数才可调用（`Engine::register_fn`），与§3.1既定"仅白名单API可调用，无法反射/枚举宿主进程内其他任何符号"的约束天然吻合，无需额外裁剪语言特性。
- **纯Rust无FFI边界**：与RGS-BAS-008核心SDK/服务端统一采用Rust不同，Lua子集方案（如`mlua`）需经C FFI边界，引入RGS-DTL-008§已确立的FFI安全校验负担（panic捕获、内存所有权跨界）；Rhai作为纯Rust crate直接编译进宿主进程，消除该类边界风险，符合"资源受限沙箱不应自带额外攻击面"的判断。
- **资源限制原生支持**：Rhai提供`Engine::set_max_operations`（时间/步数上限）、`set_max_expr_depth`、`set_max_array_size`等内置资源限制API，直接满足RGS-BAS-005"内存/时间受限"要求，无需自行实现解释器层面的资源计量。

绑定机制：§3.1既有`register_whitelisted_fn`接口契约以Rhai的`Engine::register_fn`为具体实现载体——沙箱初始化时仅调用该函数注册白名单集合，不调用`Engine::register_global_module`等会引入额外符号面的API。

license确认：Rhai MIT许可，OSI认可，无CON-001顾虑。

---

## 8. 本文档的覆盖范围与后续计划

本文档覆盖：`plugin_registry`/`plugin_audit`两表物理DDL（含经济类标记的部分索引）、沙箱脚本白名单API的Rust接口设计与永久事实强制路由的类型级强制机制、生命周期状态转移的完整合法性表与OCC实现、经济类插件单点判定与非经济类插件ARC-016通道复用的双路径算法、熔断与指数退避的具体计算公式、**沙箱脚本引擎最终选型（TBD-PLG-001：Rhai）**。
...
| TBD-PLG-001（沙箱脚本引擎选型） | §7 |
| AC-PLG-001（热插拔+跨节点同步延迟，NFR-PLG-003 p99<5s） | §4（状态机/触发时序/OCC实现）+ §5.1（复用ARC-016既有分发通道，非独立轮询；p99<5s 时限依 BAS-005§7 既有收敛时限约束，本 DTL 仅落实算法层） |
| AC-PLG-002（沙箱异常故障注入,宿主不崩溃） | §3.1（白名单API注册接口，脚本无法反射/枚举宿主符号）+ §3.2（永久事实强制封装,异常不得传播为宿主进程异常）+ §6（熔断+指数退避,故障隔离设计） |
| AC-PLG-003（版本回滚演练） | §2（`script_ref`保留历史版本引用，`version_seq`OCC）+ §4（状态机涵盖回滚路径：已启用→已禁用；已弃用→已移除含FR-PLG-012归档决定）+ §5.1（复用ARC-016既有"可立即回退至上一版本"机制，FR-GOV-023） |
| AC-PLG-004（移除演练,无孤儿表遗留） | §2（`plugin_audit`不可变留痕,`idx_plugin_audit_plugin_id_occurred`支撑取证查询）+ §4（`已弃用→已移除`状态转移前置校验FR-PLG-012数据归档决定已完成） |
