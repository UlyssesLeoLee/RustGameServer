...
**反作弊与作弊治理体系：物理数据库设计・事件线格式・案件聚合算法详细设计**
...
1. [前言](#1-前言)
2. [物理数据库设计：admin_db反作弊三表](#2-物理数据库设计admin_db反作弊三表)
3. [事件线格式](#3-事件线格式)
...
本文档覆盖：`admin_db`内反作弊三表的物理DDL（含分区/索引/OCC乐观锁列）、`DetectionSignalRaised`/`PlayerReportSubmitted`两个事件的具体线格式、案件聚合主流程与简单规则置信度计算的完整伪代码（含并发冲突处理）、TBD-ANT-001四项参数的初始默认值提案。
...
| RGS-BAS-025§4.2 智能层分析图接入 | §4.2（触发条件）、§6（明确排除内部实现） |
| TBD-ANT-001（附件D） | §5 |
