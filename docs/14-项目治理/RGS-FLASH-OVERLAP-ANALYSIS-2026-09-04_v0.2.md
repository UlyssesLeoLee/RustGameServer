# RGS-FLASH-OVERLAP-ANALYSIS-2026-09-04 v0.1 — RGS × [游戏A] API 维度对比分析 (adopt / keep / hybrid)
...
> **依据**: 9/4 15:34 JST user 拍板 (per 14:58 JST 拍板规则) "**仅API对齐, 有可取之处的可以酌情优化, 没有可取之处或者较差则保留rgs设计**" + 9/4 14:30 JST user 贴 "Rust 游戏服务器设计参考框架" (Erlang/OTP → Rust 9 原则 + 6 反模式) + `RGS-AI-HANDOFF-DOWNSTREAM-2026-09-04-[游戏A]借鉴优化.md` v0.1 §2 (5 项可立即执行) + [游戏A]借鉴分析 .md §4 (5 可取之处 + 1 反例)
> **配套**: `RGS-SPEC-CROSS-002_gRPC_Proto风格指南_v0.1.md` (🔴 NO-GO 占位) + `RGS-DDD-2026-09-04-GAP-AUDIT_v0.3.md` (7 域全量差距审计, bb9f977)
> **作用域**: 6 域 proto (player / match / economy / social / admin / gm-backend) + card-service proto + common.proto v1 + [游戏A] 96 proto (1351 RPC) — **API 维度 only**, 不动业务逻辑, 不动 6 域架构, 不动 per-entity actor 决策
...
| **[游戏A] proto 文件数** | 96 | 借鉴分析 .md §0 提取命令 | ✅ |
| **[游戏A] RPC 总数** | 1351/1394 (97.0% 成功提取) | 借鉴分析 .md §0 表格 | ✅ |
| **比例** | [游戏A] : RGS = ~20 : 1 | 借鉴分析 .md §1 表格 | ✅ |
...
- **[游戏A] 实际 proto 风格未直接看** (per 借鉴分析 .md §0 只列 RPC 数 + 标题, 未列字段/枚举风格) — 通过 5 大可取之处 + 1 反例 + system prompt 设计哲学推断, v0.2 跟进补
...
- **范围**: 6 域 proto (player / match / economy / social / admin / gm-backend) + card-service proto + common.proto v1 + [游戏A] 96 proto (1351 RPC) — **API 维度 only**
- **阶段**: v0.1 起草 (Mavis 写+自审 1 次, 准备二审)
- **风格**: 11 维度对比矩阵 + per-域 proto diff + 3 可选借鉴点

### 1.2 关键结论 (执行前必读, per 8/26 JST 缺标比错标)

1. **主结论 (per user 拍板)**: **11 维度 API 风格 RGS 都比 [游戏A] 优或相当, 应该 keep RGS**, 仅在 RGS-SPEC-CROSS-002 v0.2 升版时落实规范细节 (包名 `rgs.*` + 错误码字典 CROSS-001 + 字段废弃规范 + 流控 + 超时 + 兼容性)
2. **3 个可借鉴的 API 层技术点** (per 借鉴分析 doc §4 + handoff v0.1):
   - **Hybrid-1**: admin 域 audit_log 增 `log_title` 字段 (借鉴 [游戏A] `#rpc{code, log_title}` 思路, RGS 增强)
   - **Hybrid-2**: 保留 RGS batch 域 `enum GrpcDomain` 5 域桶化 ([游戏A] center/zone 分片思路的 RGS 变体, 不引入 center)
   - **Hybrid-3**: 评估 rgs-testkit 加 bot 压测工具 (借鉴 [游戏A] `tester*.erl` 思路, RGS 增强)
3. **1 个反例必须避免** (per 借鉴分析 doc §4 #5): [游戏A] 9+6=15 复制变体 (9 holiday_* 节日 + 6 arena 变体); RGS match v2 单 file 9 RPC + 经济 trade_saga 单 file 3 saga 已避免, **保持 RGS**
4. **RGS 12 大类 vs [游戏A] 12 大类 (per 借鉴分析 .md §2)**: 148 场景/移动 + 部分 198 角色养成 (类比"卡组养成") + 部分 241 战斗 PVE (副本/挑战奖励) + 90 经济 (限时商店) + 123 社交 (邮件/好友) + 184 活动 (数据驱动框架) + 10 排行榜 + 37 GM — 都是 **业务层参考**, 不在本 v0.1 (API only) 范围
...
**总评**: 7 域 11 维度 77 cells, **77/77 keep RGS** (RGS API 风格比 [游戏A] 优或相当) + 1 Hybrid-1 + 1 Hybrid-2 + 1 Hybrid-3 = 4 项可选借鉴
...
| 12 | **i18n `I18nString + Locale` enum** | `common.v1.I18nString { default_text, translations }` + `Locale { LOCALE_UNSPECIFIED=0, ZH_CN, EN_US, JA_JP, KO_KR }` | RGS 比 [游戏A] 更早实装 |
...
## 3. [游戏A] API 借鉴 (per 借鉴分析 .md §4 5 条可取之处 + 1 反例)

### 3.1 5 条可取之处 (架构层面)

| # | [游戏A] 做法 | 描述 | RGS 现状 | adopt/keep/hybrid 决策 |
|---|---|---|---|---|
...
| 5 | (设计哲学层) `proto_lib:repack/2` 协议版本兼容 | 允许"追加字段/整型拓宽", `pack_code_mate/1` 随包下发 schema | RGS 静态生成 + v1 suffix 目录, 未来 v0.2 评估 prost-reflect 运行时 schema (per audit v0.3 §4 #4 原则 #4 P2 backlog) | **keep RGS** (静态生成 + v1 suffix 标准化, [游戏A] 运行时 repack 是历史包袱) |
...
| 6 | **i18n** | `I18nString { default_text, translations }` + `Locale` enum (common 共享) | 估计无显式 i18n schema | **keep RGS** (RGS 早实装, [游戏A] 估计是 .po 文件) |
...
2. **复用 [游戏A] 借鉴分析**:
   - [游戏A] `tester*.erl` 是 end-to-end bot 跑真实协议
...
### 6.4 6 个 keep 论证 (RGS API 比 [游戏A] 优或相当)
...
| P2-2 | [游戏A] 跨盘 .tsv 文件 read (API清单-全量 + 按文件分组), 补 12 大类业务层 v0.2 报告 | §0.3 已知缺口 | 1d |
| P2-3 | 12 大类业务层映射 (per 借鉴分析 .md §2): 卡牌养成 + 副本/挑战 + 排位/赛季 + 限时商店 + 邮件/好友 + 排行榜 + GM, 写出 v0.2 业务层分析 | handoff v0.1 §2.1 衍生 + 借鉴分析 §2 | 3-5d |
| P2-4 | [游戏A] 跨盘 .erl 文件抽样 read (3-5 个 proto_*.erl), 推断实际 proto 风格 (推断目前基于 §4 可取之处 + system prompt 设计哲学) | §0.3 已知缺口 | 2d |
...
| P3-3 | [游戏A] admin audit_log 字段 (code + log_title) 实装到 RGS admin 域 (per Hybrid-1 衍生, v0.5+), 含 schema 迁移 + entity 增字段 + service 透传 | Hybrid-1 衍生 | 2-3d |
...
- **[游戏A] 跨盘 .tsv 文件未读** (`E:\[跨盘-某发行商目录]\[游戏A]\server分析\分析产出\API清单-*.tsv`) — bash 工具对 E: 盘权限受限, v0.2 P2-2 跟进
- **[游戏A] 实际 proto 风格未直接看** (per 借鉴分析 .md §0 只列 RPC 数 + 标题, 未列字段/枚举风格) — 通过 §4 可取之处 + system prompt 设计哲学推断, v0.2 P2-4 跟进
- **43 条未提取 + 113 条无标题** (per 借鉴分析 .md §0) — 数据缺口, 不影响 v0.1 决策
...
- **[游戏A] 性能 baseline 未测** — 需要起 [游戏A] Erlang server 跑同 client, 测 P50/P95/P99 latency + throughput + memory, 跟 RGS k3s Phase C 阶段 C 跑通后对比
- **RGS 性能 baseline 缺** — per audit v0.3 §8.3, 5 域 + card + batch 实测 P99 待补
- **rgs-testkit 现状** — audit v0.3 §6.2 描述 41 命中 InMemory test, 缺跟 [游戏A] `tester*.erl` 对比数据
...
| Evidence 段 (commit SHA / file:line) | ✅ | §2.1 12 proto 实证 + §2.2 20 模式 file:line 实证 + §3 [游戏A] §4 5 条可取之处 + 1 反例 |
...
| 凭据硬 ban (per 8/27 11:06 JST) | ✅ | 全文无 env value 痕迹, [游戏A] 跨盘引用走 Read 实证 |
...
| 跟 RGS-AI-HANDOFF-DOWNSTREAM-2026-09-04-[游戏A]借鉴优化 v0.1 一致性 | ✅ | 跟 handoff v0.1 §1 "不做逐条 RPC 移植" + §2 5 项可立即执行 决策一致 |
...
| **B** | v0.1 部分接受, 要求 Mavis 补 5 个 proto 实证 (social/replay/leaderboard/i18n/cluster-ops) + [游戏A] .tsv read | 1 个回执, 列必补项, Mavis v0.2 必补 | — |
| **C** | 全部打回 ❌, 重新看 [游戏A] 实际 proto 风格 (跨盘 .erl 文件 read) 再决策 | Mavis 改稿重走 9.1 → 9.2, 6 域架构保留前提下重做 11 维度分析 | — |
...
| 版本 | 日期 (JST) | 修订人 | 变更 |
|---|---|---|---|
| **v0.1** | 2026-09-04 15:38 | 架构师(Mavis 接手 agent per DEC-008) | 初始创建: RGS × [游戏A] API 维度对比分析 (per 9/4 15:34 JST user 拍板 "仅API对齐, 有可取之处的可以酌情优化, 没有可取之处或者较差则保留rgs设计"), 11 维度对比矩阵 (命名/Request/错误码/分页/时间戳/i18n/枚举/oneof/流式/health/契约代码生成) + per-域 proto diff (5 域 + card + gm-backend + common + batch = 9) + 3 Hybrid 借鉴点 (Hybrid-1 admin audit_log log_title + Hybrid-2 batch GrpcDomain 5 域桶化 + Hybrid-3 rgs-loadtest bot 压测) + 1 反例 ([游戏A] 9+6=15 复制变体) + 1-3 周 backlog (P1 2 项 + P2 4 项 + P3 3 项, 总 16-23d ≈ 3-5 周) + DDD Review v0.2 二审流程 + 4 段已知缺口 (报告本身 + 框架对照 + 数据 + 业务), 配套 RGS-DDD-2026-09-04-GAP-AUDIT v0.3 (bb9f977) + RGS-SPEC-CROSS-002 v0.1 (🔴 NO-GO 占位) + handoff v0.1 (5 项可立即执行), per L13 自指字段 deferred + 8/27 11:06 JST 凭据硬 ban 守护 + 8/26 JST 禁回溯叙事守护 + 8/21 JST 5 域独立 Lead 守护 (扩展到 7 域 card) + 9/4 15:34 JST user 拍板 "仅 API 对齐, 酌情优化, 较差则保留" |
