...
**RustGameServer 分布式游戏服务器 — 主要技术选型 v0.5**
...
| 配套文档 | 19 份 ADR（RGS-ADR-0001～0054）、RGS-OPS-001 部署运维说明、RGS-IMPL-001 实施约定、各域 BAS/DTL、RGS-QA-001 v0.9、RGS-PLAN-001 v0.6 |
| 制定日 | 2026-08-19 |
| 最终更新日 | 2026-08-21 |
| 制定者 | 架构师 |
...
### 6.2.1 人类工时基线（v0.4 旧估算，仅作对比）

> **v0.5 软化**：按 RGS-ADR-0025 公式，**已决选型**总 OLU 估算（人·天/周）= **19 人·天/周 ≈ 4 SRE 等效全职**。NFR-OP-010 = 2 SRE 团队（≤ 20 人·天/周）**已逼近上限**。此估算在**纯人类开发**场景下成立；**AI 辅助开发**场景下单位失精度，详见 §6.2.2。

### 6.2.2 token-OLU 框架（v0.5 新增，per user preference 2026-08-21）

> **核心问题**：AI 辅助开发场景下，AI 在上下文窗口内可秒级生成数百行 Rust 代码；人类工作日含会议 / 上下文切换 / 决策等待等开销。**人天单位在 AI 协作场景下失去精度**——"1 SRE 团队 20 人·天/周" 与 "AI 协作下 1 SRE 团队 1 周产出" 非线性对应。
>
> **决策**：OLU 单位从"人·天"改为"token"。具体换算公式与上下界如下（**待 SRE Lead + PM 在 RGS-PLAN-001 v0.6 §3.5 环境核验 + 真实 AI 协作会话中校准**）：
...
| 类别 | 组件数 | v0.4 估算（人·天/周） | v0.5 估算（**tokens/周**）| 备注 |
|---|---:|---:|---:|---|
...
> **决策依据**：RGS-QA-001 v0.9 §0 DEC-005（5 域独立 Lead）+ DEC-006（路径 B 14-18 周窗口）。5 域 Lead 各配独立 AI 协作流（5 个独立上下文窗口 / 决策路径 / 代码审查 / 测试生成）。
>
> **粗算（待 SRE Lead + PM 校准）**：
...
> **对比 v0.4 19 人·天/周 ≈ 4 SRE**：5 域独立 Lead 配置下 token 周均 ~12M-26M，是 v0.4 已决选型 token 周均（~19M-30M）的 **0.4-1.4 倍**。**8-12 周窗口下此值是 1.5-3.5 倍**（per DEC-006 路径 B 拉长到 14-18 周已显著降低周均 token 压力）。

### 6.2.3 NFR-OP-010 重定义（v0.5）

> **v0.5 软化**：原 NFR-OP-010 = "2 SRE 团队 ≤ 20 人·天/周"（人类工时基线）**在 AI 辅助开发场景下不再适用**。重定义为：

| 约束维度 | 旧（v0.4 人·天/周）| 新（v0.5 token/周） | 实际意义 |
|---|---|---|---|
| 2 SRE 团队上限 | 20 人·天/周 | **~20M tokens/周**（按 1M tokens/人·周换算）| 5 域独立 Lead + 2 SRE 总 token 上限 |
| 5 域 Lead 编制 | 兼任架构师 / SRE（v0.4 方案）| **5 域独立 Lead × ~2M-8M/域/周** = ~14M-26M/周（v0.5 估算）| 5 域独立配置；总 token 周均翻倍 |
| 上下文窗口约束 | 不适用 | **单会话 200K-1M tokens** | 超过需重启会话或滑动窗口 |
| 单次决策质量 | 不适用 | **多轮对话 5-20 轮 / 决策** | 决策复杂度上限 |

### 6.2.4 校准与回算路径

> **v0.5 不做实质校准**——具体 token 数字依赖实际 AI 协作会话的输入 / 输出 / 决策轮次 / 上下文窗口使用率。
>
> **校准路径**：
> 1. PH-0.5（开发前授权评审）前：SRE Lead + PM 提供 5 域独立 Lead 真实工作样本（每域 1-2 周 token 消耗记录）
> 2. PH-1（工程基础）期间：每域 Lead 跑 workspace / testkit / CI 基线；采集首周 token 实际值
> 3. PH-3（控制面）前：SRE Lead 校准 §6.2.2.3 表区间；如区间偏差 > 30%，升 v0.6
> 4. PH-7（发布 Gate）前：OLU 实际 token vs 估算 token 偏差写入 CIR 闭环

### 6.2.5 AI 协作开发 vs 纯人类开发 OLU 对比（参考）
