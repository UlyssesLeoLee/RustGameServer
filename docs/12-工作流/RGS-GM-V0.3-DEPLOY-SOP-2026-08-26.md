...
| 版本 | **0.3**（per worker 升版任务 2026-08-26 ~22:00 JST "DEPLOY-SOP v0.2 → v0.3 升版同步 rgs-web 接 5 域 gRPC 落地"——§4 §5 19 页面 [游戏C]_src 完备表更新到 v0.3 实际（9 真实 + 5 后续 + 5 不做）+ §A.6 v0.3 升版实证段）|
...
Ulysses 要求"和 [游戏C]_src 一样备齐,需确保有效"= **5 域 gRPC 必须真实跑通**,rgs-web 接真实 client。
...
6. 19 页面 [游戏C]_src 完备表(commit + 报告)
...
**9 个页面 v0.3 真实**（Dashboard / Servers / Players / Live Console / Config / Hot Update / Operations SQL / Docs & Health / Worktrees）+ **1 个部分落地**（Reports metrics 9464 端口不可达降级）。其他 9 个 [游戏C]_src 页面（Items / Mall / Support / TaskGroupBuilder / Accounting / Login / PermissionManagement / OaApprovals / PaymentAnalytics / Canvas）的实际状态见 §5 19 页面 [游戏C]_src 完备表。

---

## 5. 19 页面 [游戏C]_src 完备表(v0.3 实际)
...
- ❌ [游戏C]_src 4 高级页面(Login / RBAC / OA / Payment / Canvas)→ 一人公司不实现
...
**v0.3 落地 9 页面真实**（per §5 19 页面 [游戏C]_src 完备表 v0.3 实际状态）：
...
**5 页面不做**（per [游戏C]_src 一人公司基线 / DEC-008）：
