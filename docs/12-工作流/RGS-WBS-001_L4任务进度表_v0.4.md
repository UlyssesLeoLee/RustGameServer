...
| **WF-1-55.42** | **DTL-026 §4.1 benchmark 子任务（per RGS-OPEN-QA-001 Q-D-10 + ACTIONS-v0.3 A-10）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **撮合 benchmark 实测 n 上限 + DTL-026 §4.1 补 n≤500 占位 + 降级/熔断策略（merge commit `22dd047`）** |
| **WF-1-55.43** | **RGS-DEC-Q003 跨 DB Saga 审批包（per RGS-OPEN-QA-001 Q-M-01 + ACTIONS-v0.3 B-02）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **依赖 B-01（WF-1-55.43 应在 DTL-015/016 §3.4 步骤编号完成后 start）；6 场景决议 + RACI + DTL-031 §8.2 解除阻断（merge commit `2960d7f`）** |
| **WF-1-55.44** | **4 域 rgs-testkit dev-dep + 集成测试骨架（per RGS-OPEN-QA-001 Q-M-02 + ACTIONS-v0.3 B-03）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **player/match/social/admin 4 域各补 rgs-testkit dev-dep + 1 份 tests/integration_*.rs（参考 economy 现有模板）；~4×0.5 人·天（merge commit `b5a08a3`）** |
| **WF-1-55.45** | **OTel 启用 + NATS traceparent + sqlx-tracing + 5 域 OTLP 出口（per RGS-OPEN-QA-001 Q-M-03 + ACTIONS-v0.3 B-04）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **核实 53.12/54.13 状态 + NATS header 注入 traceparent + sqlx-tracing 10-20% 采样 + 5 域各自直接 OTLP（merge commit `396f56a`）** |
| **WF-1-55.46** | **verify_probe_consistency.ps1 CI 脚本（per RGS-OPEN-QA-001 Q-M-04 + ACTIONS-v0.3 B-05）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **6 份 manifest probe 段结构化 diff + 阈值一致性全 6 份核对（不是抽查 2 份）+ CI 接入（merge commit `a0d7395`）** |
| **WF-1-55.47** | **reservation IT + 混沌测试 + span 断言（per RGS-OPEN-QA-001 Q-M-07 + ACTIONS-v0.3 B-08）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **it_reservation_*.rs（create→conflict→release/cleanup）+ 混沌测试（DB 断开/死锁 P1，row 外部 DELETE P2）+ span 三层断言（merge commit `8c9fddd`）** |
| **WF-1-55.48** | **verify_fail_closed.ps1 + CI 接入 + RGS-TS-001 §5 状态改（per RGS-OPEN-QA-001 Q-M-08 + Q-M-10 + ACTIONS-v0.3 B-09）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **固化 fail-closed 验证 + 接入 CI（每次 manifest/RBAC 变更 PR 触发）+ TS-001 §5 改"已决策：NATS JetStream"（merge commit `0a11923`）** |
| **WF-1-55.49** | **新建 RGS-ADR-0055 DEC-005/008 兼容论证 + RACI 简表（per RGS-OPEN-QA-001 Q-G-01 + Q-G-02 + ACTIONS-v0.3 C-01）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-26** | **ADR-0055 v0.1 + RGS-PLAN-001 v1.0 §1.2 补 RACI 简表（4 类决策：代码合并/DTL 升版/生产发布/资金相关）（merge commit `577f9b9`）** |
| **WF-1-55.50** | **WBS-001 §8 log 模板 + 11 份 log 逐份核验（per RGS-OPEN-QA-001 Q-G-04 + ACTIONS-v0.3 C-03）** | **Ulysses(per DEC-008)** | **done** | **100%** | **2026-08-24** | **B-CODE/C-CODE log 新模板（强制验证证据字段）+ 7 G-CODE + 4 B-CODE = 11 份 log 逐份核验报告（merge commit `5d030b5`）** |
