# H5 [游戏A]_client → RGS 完整迁移矩阵

**核心** (per 2026-09-09 15:25 JST Ulysses 拍板): H5 源码完整在 `E:\[跨盘-某发行商目录]\[游戏A]\server分析\[游戏A]_client_h5\temp\quick-scripts\src\assets\Scripts\`。如果每个端口都跟 erlang 一致, 理论可完美迁移.

## 1. 真实规模 (per proto_mate.js)

| 维度 | 数量 | 来源 |
|------|------|------|
| H5 [游戏A]_client send (cli → srv) | **766 unique cmd** | `temp/quick-scripts/src/assets/Scripts/net/proto_mate.js` |
| H5 [游戏A]_client recv (srv → cli) | **875 unique cmd** | 同上 |
| shim 已实现真 [游戏A] cmd | **10** | 1.3% 业务覆盖 |
| shim-internal RGS 测试 | 2 (10400, 11001) | 不计业务覆盖 |
| [游戏A]_server proto_*.erl | 43 文件 | `[游戏A]_server/src/proto/proto_101.erl` ~ `proto_239.erl` |
...
| [游戏A]_client 全资源 | 3025 文件 81.48 MB | `[游戏A]_client/res/resource/` (含 plist+png+mp3+mp4) |
| Cocos Creator 项目 | 2.3.2 (项目文件+settings+CusEngine) | `[游戏A]_client_h5/` |
...
H5 真能 build — 完整 Cocos Creator 2.3.2 project 在 `E:\[跨盘-某发行商目录]\[游戏A]\server分析\[游戏A]_client_h5\`:
...
2. 导入 `[游戏A]_client_h5/` 作为 Cocos Creator project
3. 导入 `[游戏A]_client/res/resource/*` (3025 文件 81MB) 作为 assets
4. `cocos compile -p web -m release` → 输出 `build/web-mobile/`
5. 部署到 `D:\[游戏A]-h5-orig\public\[游戏A]-real\` 替换 HTML
6. rgs-shim-rust 8001 接收真 [游戏A]_client 766 cmd
...
### 6.1 协议级 (Node.js, 字节级)
- `bench/proto-test.js` — 17 case 真 [游戏A]_client 协议模拟, 17/17 passed

### 6.2 资源级 (Playwright, 真 [游戏A] 资源)
- `tests/05-[游戏A]-battle-flow.spec.ts` — 10 case 真 [游戏A] 资源 + 真实 SmartSocket 协议, 10/10 passed, 11 张截图

### 6.3 H5 渲染级 (待 build)
- 完整 Cocos Creator 2.3.2 build, 部署到 web 8083
- [游戏A]_client H5 启动 → 调 shim 9001 → 真实 RGS 5 域 gRPC → 真数据
- (待 shim 100% 业务覆盖后实施)
...
| 约束 | 说明 | 影响 |
|------|------|------|
| [游戏A]_client H5 binary 不在 | `project.9b183.js` 缺, settings.js 缺 | 需 Cocos Creator 2.3.2 build |
| [游戏A]_client/res vs H5 资源格式 | res/ 是 executable 格式 (plist/png/mp3), H5 需 Cocos Creator 导入 | 81MB 资源需转换 |
...
5. **Phase 5 (H5 build)**: Cocos Creator 2.3.2 build [游戏A]_client H5, 部署 + 真实渲染 (1-2h build)
